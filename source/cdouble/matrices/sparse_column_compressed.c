// Automatically generated from source/cfloat/matrices/sparse_column_compressed.c on Fri Dec  1 18:48:06 2023
// Automatically generated from source/cdouble/matrices/sparse_column_compressed.c on Fri Dec  1 17:36:03 2023
//
...
static uint32_t ccs_get_column_entries(const jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t** pp_indices, _Complex double** pp_values)
{
    uint32_t offset, len;
    if (col == 0)
    {
 ...
 */
static jmtx_result ccs_insert_entry_at(jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t position, _Complex double value, uint32_t index)
{
...
jmtx_result jmtxz_matrix_ccs_new(
        jmtxz_matrix_ccs** p_mtx, uint32_t cols, uint32_t rows, uint32_t reserved_entries,
        const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    jmtxz_matrix_ccs* const this = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*this));
    if (!this)
    {
...
jmtx_result jmtxzs_matrix_ccs_new(
        jmtxz_matrix_ccs** p_mtx, uint32_t cols, uint32_t rows, uint32_t reserved_entries,
        const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!p_mtx)
    ...
    return jmtxz_matrix_ccs_new(p_mtx, cols, rows, reserved_entries, allocator_callbacks);
}
...
jmtx_result jmtxzs_matrix_ccs_destroy(jmtxz_matrix_ccs* mtx)
{
    ...
    if (mtx->base.type != JMTXZ_TYPE_CCS)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    jmtxz_matrix_ccs_destroy(mtx);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_ccs_shrink(jmtxz_matrix_ccs* mtx)
{
    ...
    return jmtxz_matrix_ccs_shrink(mtx);
}

jmtx_result jmtxz_matrix_ccs_set_col(jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t n, const uint32_t* indices, const _Complex double* values)
{
...
jmtx_result jmtxzs_matrix_ccs_set_col(jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t n, const uint32_t* indices, const _Complex double* values)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxz_matrix_ccs_set_col(mtx, col, n, indices, values);
}

void jmtxz_matrix_ccs_vector_multiply(const jmtxz_matrix_ccs* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
...
jmtx_result jmtxzs_matrix_ccs_vector_multiply(const jmtxz_matrix_ccs* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxz_matrix_ccs_vector_multiply(mtx, x, y);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_ccs_set_entry(jmtxz_matrix_ccs* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    ...
    return jmtxz_matrix_ccs_set_entry(mtx, i, j, value);
}
...
jmtx_result jmtxzs_matrix_ccs_get_entry(const jmtxz_matrix_ccs* mtx, uint32_t i, uint32_t j, _Complex double* p_value)
{
    ...
    *p_value = jmtxz_matrix_ccs_get_entry(mtx, i, j);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxz_matrix_ccs_get_col(const jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t** p_indices, _Complex double** p_elements)
{
    return ccs_get_column_entries(mtx, col, p_indices, p_elements);
}

jmtx_result jmtxzs_matrix_ccs_get_col(const jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t* n, uint32_t** p_indices, _Complex double** p_elements)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *n = jmtxz_matrix_ccs_get_col(mtx, col, p_indices, p_elements);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_ccs_count_values(const jmtxz_matrix_ccs* mtx, _Complex double v, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_ccs_count_values(mtx, v);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_ccs_count_indices(const jmtxz_matrix_ccs* mtx, uint32_t v, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_ccs_count_indices(mtx, v);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxz_matrix_ccs_apply_unary_fn(const jmtxz_matrix_ccs* mtx, int (*unary_fn)(uint32_t i, uint32_t j, _Complex double* p_element, void* param), void* param)
{
...
jmtx_result jmtxzs_matrix_ccs_apply_unary_fn(const jmtxz_matrix_ccs* mtx, int (*unary_fn)(uint32_t i, uint32_t j, _Complex double* p_element, void* param), void* param)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxz_matrix_ccs_apply_unary_fn(mtx, unary_fn, param);
}
...
jmtx_result jmtxzs_matrix_ccs_remove_zeros(jmtxz_matrix_ccs* mtx)
{
    ...
    jmtxz_matrix_ccs_remove_zeros(mtx);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_ccs_remove_bellow(jmtxz_matrix_ccs* mtx, double v)
{
    ...
    jmtxz_matrix_ccs_remove_bellow(mtx, v);
    return JMTX_RESULT_SUCCESS;
}
...
uint32_t jmtxz_matrix_ccs_elements_in_row(const jmtxz_matrix_ccs* mtx, uint32_t row)
{
    ...
    {
        uint32_t* col_indices;
        _Complex double* unused_col_values;
...
jmtx_result jmtxzs_matrix_ccs_elements_in_row(const jmtxz_matrix_ccs* mtx, uint32_t row, uint32_t* p_n)
{
    ...
    *p_n = jmtxz_matrix_ccs_elements_in_row(mtx, row);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxz_matrix_ccs_get_row(
    const jmtxz_matrix_ccs* mtx, uint32_t row, uint32_t n, _Complex double* p_values, uint32_t* p_columns)
{
    uint32_t k = 0;
...
jmtx_result jmtxzs_matrix_ccs_get_row(
        const jmtxz_matrix_ccs* mtx, uint32_t row, uint32_t n, _Complex double* p_values, uint32_t* p_count, uint32_t* p_columns)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_count = jmtxz_matrix_ccs_get_row(mtx, row, n, p_values, p_columns);
    return JMTX_RESULT_SUCCESS;
}
                                      ...
                                      const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    jmtxz_matrix_ccs* const out = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*out));
    if (!out)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
    ...
    _Complex double* const new_elements = mtx->base.allocator_callbacks.alloc(mtx->base.allocator_callbacks.state, (n_elements) * sizeof*new_elements);
    if (!new_elements)
    {
        mtx->base.allocator_callbacks.free(mtx->base.allocator_callbacks.state, row_cum_counts);
    ...
    *row_cum_counts = 0;
    ...
    {
        n = jmtxz_matrix_ccs_elements_in_row(mtx, j);
        uint32_t c = jmtxz_matrix_ccs_get_row(mtx, j, n, new_elements + p, new_indices + p);
        assert(c == n);
        p += n;
                                      ...
                                      const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    return jmtxz_matrix_ccs_transpose(mtx, p_out, allocator_callbacks);
}

jmtx_result jmtxz_matrix_ccs_copy(const jmtxz_matrix_ccs* mtx, jmtxz_matrix_ccs** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
        allocator_callbacks = &mtx->base.allocator_callbacks;
    ...
    _Complex double* const elements = mtx->base.allocator_callbacks.alloc(mtx->base.allocator_callbacks.state, (mtx->n_entries) * sizeof *elements);
    if (!elements)
    {
...
jmtx_result jmtxzs_matrix_ccs_copy(const jmtxz_matrix_ccs* mtx, jmtxz_matrix_ccs** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxz_matrix_ccs_copy(mtx, p_out, allocator_callbacks);
}

void jmtxz_matrix_ccs_zero_all_elements(const jmtxz_matrix_ccs* mtx)
{
    memset(mtx->values, 0, sizeof(*mtx->values) * mtx->n_entries);
}

jmtx_result jmtxzs_matrix_ccs_zero_all_elements(const jmtxz_matrix_ccs* mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxz_matrix_ccs_zero_all_elements(mtx);

    return JMTX_RESULT_SUCCESS;
}

void jmtxz_matrix_ccs_set_all_elements(const jmtxz_matrix_ccs* mtx, _Complex double x)
{
...
jmtx_result jmtxzs_matrix_ccs_set_all_elements(jmtxz_matrix_ccs* mtx, _Complex double x)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxz_matrix_ccs_set_all_elements(mtx, x);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result
jmtxz_matrix_ccs_build_col(jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t n, const uint32_t* indices, const _Complex double* values)
{
