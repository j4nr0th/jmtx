...
jmtx_result jmtxz_matrix_brm_new(
        ...
        const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    return jmtxz_matrix_brm_new(p_mtx, cols, rows, ubw, lbw, set_value, allocator_callbacks);
}
...
void jmtxz_matrix_brm_set_row(const jmtxz_matrix_brm* mtx, uint32_t row, _Complex double values[])
{
...
jmtx_result jmtxzs_matrix_brm_set_row(const jmtxz_matrix_brm* mtx, uint32_t row, _Complex double values[])
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
void jmtxz_matrix_brm_vector_multiply(const jmtxz_matrix_brm* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
...
jmtx_result jmtxzs_matrix_brm_vector_multiply(const jmtxz_matrix_brm* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxzs_matrix_brm_get_row(const jmtxz_matrix_brm* mtx, uint_fast32_t* n, uint32_t row, _Complex double* p_elements[1])
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
uint32_t jmtxz_matrix_brm_entries_in_col(const jmtxz_matrix_brm* mtx, uint32_t col)
{
    return jmtxz_matrix_brm_last_pos_in_col(mtx, col) - jmtxz_matrix_brm_first_pos_in_col(mtx, col) + 1;
}

jmtx_result jmtxzs_matrix_brm_entries_in_col(const jmtxz_matrix_brm* mtx, uint32_t col, uint32_t* p_n)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_n = jmtxz_matrix_brm_entries_in_col(mtx, col);
    return JMTX_RESULT_SUCCESS;
}

uint32_t
jmtxz_matrix_brm_get_col(const jmtxz_matrix_brm* mtx, uint32_t col, _Complex double values[])
{
...
jmtx_result jmtxzs_matrix_brm_get_col(
        const jmtxz_matrix_brm* mtx, uint32_t col, uint32_t* p_count, _Complex double p_values[])
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxz_matrix_brm_transpose(
        const jmtxz_matrix_brm* mtx, jmtxz_matrix_brm** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!allocator_callbacks)
    {
...
jmtx_result jmtxzs_matrix_brm_transpose(
        const jmtxz_matrix_brm* mtx, jmtxz_matrix_brm** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxz_matrix_brm_copy(const jmtxz_matrix_brm* mtx, jmtxz_matrix_brm** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!allocator_callbacks)
    {
        allocator_callbacks = &mtx->base.allocator_callbacks;
    ...
    _Complex double* const elements = allocator_callbacks->alloc(allocator_callbacks->state, (n_entries) * sizeof (*elements));
    if (!elements)
    {
        allocator_callbacks->free(allocator_callbacks->state, out);
        return JMTX_RESULT_BAD_ALLOC;
...
jmtx_result jmtxzs_matrix_brm_copy(const jmtxz_matrix_brm* mtx, jmtxz_matrix_brm** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxzs_matrix_brm_vector_multiply_row(const jmtxz_matrix_brm* mtx, const _Complex double* restrict x, uint32_t i, _Complex double* restrict p_r)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxzs_matrix_brm_set_col(const jmtxz_matrix_brm* mtx, uint32_t col, const _Complex double* values)
{
    ...
    const uint_fast32_t len = jmtxz_matrix_brm_entries_in_col(mtx, col);
    for (uint32_t i = 0; i < len; ++i)
    {
        if (!isfinite(creal(values[i])) || !isfinite(cimag(values[i])))
        {
