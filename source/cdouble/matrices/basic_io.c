// Automatically generated from source/cfloat/matrices/basic_io.c on Fri Dec  1 18:48:06 2023
// Automatically generated from source/cdouble/matrices/basic_io.c on Fri Dec  1 17:36:03 2023
//
// Created by jan on 23.11.2023.
//

#include "basic_io.h"
#include "sparse_column_compressed_internal.h"
#include "sparse_row_compressed_internal.h"

#include <complex.h>
#include <inttypes.h>
#include <stdio.h>

jmtx_result jmtxz_matrix_crs_to_file(const jmtxz_matrix_crs *mtx, const char *filename)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    }
    if (mtx->base.type != JMTXZ_TYPE_CRS)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    if (!filename)
    {
        return JMTX_RESULT_NULL_PARAM;
    }

    FILE *const f_out = fopen(filename, "w");
    if (!f_out)
    {
        return JMTX_RESULT_NO_FILE;
    }

    fprintf(f_out, "Matrix of type %s %" PRIu32 " x %" PRIu32 " with %" PRIu32 " entries",
            jmtx_matrix_type_to_str(mtx->base.type), mtx->base.rows, mtx->base.cols, mtx->n_entries);
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        _Complex double *elements = NULL;
        uint32_t *indices = NULL;
        const uint32_t count = jmtxz_matrix_crs_get_row(mtx, i, &indices, &elements);
        fprintf(f_out, "\nRow %" PRIu32 ":", i);
        for (uint32_t j = 0; j < count; ++j)
        {
            fprintf(f_out, "\t(%" PRIu32 ", %+.8e%+.8ej)", indices[j], creal(elements[j]), cimag(elements[j]));
        }
    }
    fprintf(f_out, "\n");

    fclose(f_out);

    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxz_matrix_crs_to_file_explicit(const jmtxz_matrix_crs *mtx, const char *filename)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    }
    if (mtx->base.type != JMTXZ_TYPE_CRS)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    if (!filename)
    {
        return JMTX_RESULT_NULL_PARAM;
    }

    FILE *const f_out = fopen(filename, "w");
    if (!f_out)
    {
        return JMTX_RESULT_NO_FILE;
    }

    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        _Complex double *elements = NULL;
        uint32_t *indices = NULL;
        const uint32_t count = jmtxz_matrix_crs_get_row(mtx, i, &indices, &elements);
        for (uint32_t j = 0, k = 0; j < mtx->base.cols; ++j)
        {
            if (k < count && indices[k] == j)
            {
                fprintf(f_out, "%+.8e%+.8ej\t", creal(elements[j]), cimag(elements[j]));
            }
            else
            {
                fprintf(f_out, "%+.8e%+.8ej\t", 0.0f, 0.0f);
            }
        }
        fprintf(f_out, "\n");
    }

    fclose(f_out);

    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxz_matrix_ccs_to_file(const jmtxz_matrix_ccs *mtx, const char *filename)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    }
    if (mtx->base.type != JMTXZ_TYPE_CCS)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    if (!filename)
    {
        return JMTX_RESULT_NULL_PARAM;
    }

    FILE *const f_out = fopen(filename, "w");
    if (!f_out)
    {
        return JMTX_RESULT_NO_FILE;
    }

    fprintf(f_out, "Matrix of type %s %" PRIu32 " x %" PRIu32 " with %" PRIu32 " entries",
            jmtx_matrix_type_to_str(mtx->base.type), mtx->base.rows, mtx->base.cols, mtx->n_entries);
    for (uint32_t i = 0; i < mtx->base.cols; ++i)
    {
        _Complex double *elements = NULL;
        uint32_t *indices = NULL;
        const uint32_t count = jmtxz_matrix_ccs_get_col(mtx, i, &indices, &elements);
        fprintf(f_out, "\nCol %" PRIu32 ":", i);
        for (uint32_t j = 0; j < count; ++j)
        {
            fprintf(f_out, "\t(%" PRIu32 ", %+.8e%+.8ej)", indices[j], creal(elements[j]), cimag(elements[j]));
        }
    }
    fprintf(f_out, "\n");

    fclose(f_out);

    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxz_matrix_ccs_to_file_explicit(const jmtxz_matrix_ccs *mtx, const char *filename)

{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    }
    if (mtx->base.type != JMTXZ_TYPE_CRS)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    if (!filename)
    {
        return JMTX_RESULT_NULL_PARAM;
    }

    FILE *const f_out = fopen(filename, "w");
    if (!f_out)
    {
        return JMTX_RESULT_NO_FILE;
    }

    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        for (uint32_t j = 0; j < mtx->base.cols; ++j)
        {
            const _Complex double v = jmtxz_matrix_ccs_get_entry(mtx, i, j);
            fprintf(f_out, "%+.8e%+.8ej\t", creal(v), cimag(v));
        }
        fprintf(f_out, "\n");
    }

    fclose(f_out);

    return JMTX_RESULT_SUCCESS;
}
