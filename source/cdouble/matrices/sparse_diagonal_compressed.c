// Automatically generated from source/cfloat/matrices/sparse_diagonal_compressed.c on Fri Dec  1 18:48:06 2023
// Automatically generated from source/cdouble/matrices/sparse_diagonal_compressed.c on Fri Dec  1 17:36:03 2023
//
...
static inline jmtx_result jmtxz_matrix_cds_diagonal_array_init(const jmtx_allocator_callbacks* allocator_callbacks,
                                                              jmtxz_matrix_cds_diagonal_array* ptr, uint32_t capacity)
{
    ...
    _Complex double** const diags = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*diags) * capacity);
    if (!diags)
    {
...
static inline jmtx_result jmtxz_matrix_cds_diagonal_array_insert(const jmtx_allocator_callbacks* allocator_callbacks,
                                                                jmtxz_matrix_cds_diagonal_array* this, uint32_t idx,
                                                                _Complex double* dia)
{
    ...
    if (this->capacity == this->count)
    {
        ...
        _Complex double** const diags = allocator_callbacks->realloc(allocator_callbacks->state, this->diagonals, sizeof(*diags) * new_capacity);
        if (!diags)
        {
            return JMTX_RESULT_BAD_ALLOC;
        }
    ...
    _Complex double** const diagonals = this->diagonals;
...
static inline _Complex double* jmtxz_matrix_cds_diagonal_array_get_ptr(const jmtxz_matrix_cds_diagonal_array* this, uint32_t idx)
{
...
static inline void jmtxz_matrix_cds_diagonal_array_free(const jmtx_allocator_callbacks* allocator_callbacks,
                                                       jmtxz_matrix_cds_diagonal_array* ptr)
{
    ...
    const int32_t p_dia_idx[JMTX_ARRAY_ATTRIB(static n_diagonals)], const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    jmtxz_matrix_cds* const mtx = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*mtx));
    if (!mtx)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
    ...
    mtx_res = jmtxz_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->sub_diagonals,
    ...
    mtx_res = jmtxz_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->super_diagonals,
    ...
    for (uint32_t i = 0; i < n_diagonals; ++i)
    {
        if (p_dia_idx[i] < 0)
        {
            _Complex double* const ptr = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*ptr) *
            ...
            jmtxz_matrix_cds_diagonal_array_insert(allocator_callbacks, &mtx->sub_diagonals, -p_dia_idx[i], ptr);
        }
        else if (p_dia_idx[i] > 0)
        {
            _Complex double* const ptr = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*ptr) *
            ...
            jmtxz_matrix_cds_diagonal_array_insert(allocator_callbacks, &mtx->super_diagonals, p_dia_idx[i], ptr);
        }
    }
    if (n_diagonals > n_sub + n_sup)
    {
        ...
        _Complex double* const main_ptr = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*main_ptr) * len_main);
        if (!main_ptr)
        {
            mtx_res = JMTX_RESULT_BAD_ALLOC;
            goto failed_adding;
    ...
    *p_mtx = mtx;
    ...
    return JMTX_RESULT_SUCCESS;
failed_adding:
    jmtxz_matrix_cds_diagonal_array_free(allocator_callbacks, &mtx->sub_diagonals);
failed_sup:
    jmtxz_matrix_cds_diagonal_array_free(allocator_callbacks, &mtx->super_diagonals);
failed_sub:
 ...
 */
void jmtxz_matrix_cds_destroy(jmtxz_matrix_cds* mtx)
{
...
/**
 * Version of jmtxz_matrix_cds_set_row which does not touch the count of entries after the current row. This is useful when
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_set_diagonal_full(jmtxz_matrix_cds* mtx, int32_t dia_idx, const _Complex double values[JMTX_ARRAY_ATTRIB(const)])
{
    uint32_t len;
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_set_diagonal_part(jmtxz_matrix_cds* mtx, int32_t dia_idx, uint32_t offset, uint32_t n,
                                              uint32_t* p_count, const _Complex double values[JMTX_ARRAY_ATTRIB(static n)])
{
    uint32_t len;
    _Complex double* const ptr = jmtxz_matrix_cds_allocate_diagonal(mtx, dia_idx, &len);
    if (!ptr)
    ...
    _Complex double* const p = ptr + offset;
 ...
 */
_Complex double* jmtxz_matrix_cds_allocate_diagonal(jmtxz_matrix_cds* mtx, int32_t dia, uint32_t* p_size)
{
    _Complex double* ptr;
    uint_fast32_t len;
    uint_fast32_t idx;
    ...
    else if (dia < 0)
    {
        ...
        ptr = jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->sub_diagonals, idx);
        if (!ptr)
        {
            ...
            const jmtx_result res = jmtxz_matrix_cds_diagonal_array_insert(
    ...
    else //if (dia_idx > 0)
    {
        ...
        ptr = jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->super_diagonals, idx);
        if (!ptr)
        {
            ptr = mtx->base.allocator_callbacks.alloc(mtx->base.allocator_callbacks.state, sizeof(*ptr) * len);
            if (!ptr)
 ...
 * @return pointer to the newly allocated diagonal, or NULL in case it failed to do so
 */
_Complex double* jmtxz_matrix_cds_allocate_zero_diagonal(jmtxz_matrix_cds* mtx, int32_t dia, uint32_t* p_size)
{
    _Complex double* ptr;
    uint_fast32_t len;
    uint_fast32_t idx;
 ...
 */
_Complex double* jmtxz_matrix_cds_get_diagonal(const jmtxz_matrix_cds* mtx, int32_t dia)
{
    if (dia == 0)
    {
    ...
    {
        return jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->sub_diagonals, -dia);
    }
    //  else if (dia > 0)
    return jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->super_diagonals, dia);
}
 ...
 */
uint32_t jmtxz_matrix_cds_entries_in_row(const jmtxz_matrix_cds* mtx, uint32_t row)
{
    uint_fast32_t k = 0;
 ...
 * small)
 */
uint32_t jmtxz_matrix_cds_get_row(const jmtxz_matrix_cds* mtx, uint32_t row, uint32_t n, _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict n)],
                                 uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict n)])
{
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count && k < n; ++i)
    {
        ...
        p_cols[k] = pos;
        const _Complex double* const diagonal = mtx->sub_diagonals.diagonals[mtx->sub_diagonals.count - 1 - i];
    ...
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count && k < n; ++i)
    {
        ...
        p_cols[k] = idx + row;
        const _Complex double* const diagonal = mtx->super_diagonals.diagonals[i];
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_set_row(jmtxz_matrix_cds* mtx, uint32_t row, uint32_t n,
                                    const _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict static n)], const uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    jmtxz_matrix_cds_zero_row(mtx, row);
    for (uint_fast32_t m = 0; m < n; ++m)
    {
        const int32_t dia = (int32_t)p_cols[m] - (int32_t)row;
        _Complex double* ptr;
        _Complex double v;
        ptr = jmtxz_matrix_cds_allocate_zero_diagonal(mtx, dia, NULL);
        if (!ptr)
        {
            return JMTX_RESULT_BAD_ALLOC;
        }
 ...
 */
jmtx_result jmtxz_matrix_cds_set_col(jmtxz_matrix_cds* mtx, uint32_t col, uint32_t n,
                                    const _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict static n)], const uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    jmtxz_matrix_cds_zero_col(mtx, col);
    for (uint_fast32_t m = 0; m < n; ++m)
    {
 ...
 */
uint32_t jmtxz_matrix_cds_entries_in_col(const jmtxz_matrix_cds* mtx, uint32_t col)
{
    uint_fast32_t k = 0;
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
 ...
 */
uint32_t
jmtxz_matrix_cds_get_col(const jmtxz_matrix_cds* mtx, uint32_t col, uint32_t n, _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict n)], uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict n)])
{
    uint_fast32_t k = 0;
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count && k < n; ++i)
    {
        ...
        p_rows[k] = pos;
        const _Complex double* const diagonal = mtx->super_diagonals.diagonals[mtx->super_diagonals.count - 1 - i];
        p_values[k] = diagonal[pos];
        k += 1;
    }
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count && k < n; ++i)
    {
        ...
        p_rows[k] = idx + col;
        const _Complex double* const diagonal = mtx->sub_diagonals.diagonals[i];
 ...
 */
uint_fast32_t jmtxz_matrix_cds_entries_in_dia(jmtxz_matrix_cds* mtx, int32_t dia)
{
    if (dia >= 0)
    {
 ...
 */
uint_fast32_t jmtxz_matrix_cds_diagonal_count(const jmtxz_matrix_cds* mtx)
{
 ...
 */
void jmtxz_matrix_cds_vector_multiply(const jmtxz_matrix_cds* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
    ...
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        ...
        const _Complex double* const vals = mtx->super_diagonals.diagonals[i];
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        ...
        const _Complex double* const vals = mtx->sub_diagonals.diagonals[i];
 ...
 */
void jmtxz_matrix_cds_set_entry(const jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    if (i == j)
    {
    ...
    else if (i > j)
    {
        ...
        _Complex double* const ptr = jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->sub_diagonals, i - j);
        ptr[j] = value;
    }
    else// if (i < j)
    {
        //  Row idx smaller than col idx -> superdiagonal
        _Complex double* const ptr = jmtxz_matrix_cds_diagonal_array_get_ptr(&mtx->super_diagonals, j - i);
        ptr[i] = value;
    }
}
 ...
 */
_Complex double jmtxz_matrix_cds_get_entry(const jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j)
{
    if (i == j)
    {
        if (mtx->main_diagonal)
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_insert_entry(jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    int32_t dia = (int32_t)j - (int32_t)i;
    _Complex double* const ptr = jmtxz_matrix_cds_allocate_diagonal(mtx, dia, NULL);
    if (!ptr)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
 ...
 * @param value value to which the value is to be added
 */
jmtx_result jmtxz_matrix_cds_add_to_entry(jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    int32_t dia = (int32_t)j - (int32_t)i;
    _Complex double* const ptr = jmtxz_matrix_cds_allocate_diagonal(mtx, dia, NULL);
    if (!ptr)
 ...
 */
uint32_t jmtxz_matrix_cds_count_values(const jmtxz_matrix_cds* mtx, _Complex double v)
{
    uint_fast32_t k = 0;
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        const _Complex double* const ptr = mtx->super_diagonals.diagonals[i];
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        const _Complex double* const ptr = mtx->sub_diagonals.diagonals[i];
 ...
 */
void jmtxz_matrix_cds_zero_all_entries(const jmtxz_matrix_cds* mtx)
{
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        _Complex double* const ptr = mtx->super_diagonals.diagonals[i];
        uint_fast32_t const idx = mtx->super_diagonals.indices[i];
        memset(ptr, 0, sizeof(*ptr) * cds_superdiagonal_length(mtx->base.cols, mtx->base.rows, idx));
    }
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        _Complex double* const ptr = mtx->sub_diagonals.diagonals[i];
        uint_fast32_t const idx = mtx->sub_diagonals.indices[i];
...
/**
 * Similar to jmtxz_matrix_cds_zero_all_entries, but slower, since it can not use memset. On the other hand, it allows for
 * the value to be other than 0
 * @param mtx matrix to set
 * @param x value to which to set all entries to
 */
void jmtxz_matrix_cds_set_all_entries(const jmtxz_matrix_cds* mtx, _Complex double x)
{
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        _Complex double* const ptr = mtx->super_diagonals.diagonals[i];
 ...
 */
void jmtxz_matrix_cds_clear(jmtxz_matrix_cds* mtx)
{
    const jmtx_allocator_callbacks allocator_callbacks = mtx->base.allocator_callbacks;
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_transpose(
        const jmtxz_matrix_cds* mtx, jmtxz_matrix_cds** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
        allocator_callbacks = &JMTX_DEFAULT_ALLOCATOR_CALLBACKS;
    }
    jmtxz_matrix_cds* tps = NULL;
    int32_t dummy = 0;
    const uint32_t new_cols = mtx->base.rows;
    const uint32_t new_rows = mtx->base.cols;
    const uint_fast32_t dia_count = jmtxz_matrix_cds_diagonal_count(mtx);
    int32_t* p_dia = &dummy;
    if (dia_count != 0)
    {
    ...
    jmtxz_matrix_cds_zero_all_entries(tps);
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        const uint_fast32_t idx = mtx->super_diagonals.indices[i];
        const int32_t new_idx = -(int32_t) idx;
        const uint_fast32_t len = cds_superdiagonal_length(mtx->base.cols, mtx->base.rows, idx);
        _Complex double const* dia_ptr = mtx->super_diagonals.diagonals[i];
        res = jmtxz_matrix_cds_set_diagonal_part(tps, new_idx, 0, len, NULL, dia_ptr);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(tps);
            return res;
        }
    }
    if (mtx->main_diagonal)
    {
        ...
        res = jmtxz_matrix_cds_set_diagonal_part(tps, 0, 0, len, NULL, mtx->main_diagonal);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(tps);
            return res;
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        ...
        _Complex double const* dia_ptr = mtx->sub_diagonals.diagonals[i];
        res = jmtxz_matrix_cds_set_diagonal_part(tps, new_idx, 0, len, NULL, dia_ptr);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(tps);
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_copy(const jmtxz_matrix_cds* mtx, jmtxz_matrix_cds** p_out,
                                 const jmtx_allocator_callbacks* allocator_callbacks)
{
    jmtxz_matrix_cds* cpy = NULL;
    int32_t dummy = 0;
    const uint32_t new_cols = mtx->base.rows;
    const uint32_t new_rows = mtx->base.cols;
    const uint_fast32_t dia_count = jmtxz_matrix_cds_diagonal_count(mtx);
    ...
    for (uint_fast32_t i = 0; i < mtx->super_diagonals.count; ++i)
    {
        ...
        _Complex double const* dia_ptr = mtx->super_diagonals.diagonals[i];
        res = jmtxz_matrix_cds_set_diagonal_full(cpy, idx, dia_ptr);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(cpy);
            return res;
        }
    }
    if (mtx->main_diagonal)
    {
        const uint_fast32_t len = cds_subdiagonal_length(mtx->base.cols, mtx->base.rows, 0);
        res = jmtxz_matrix_cds_set_diagonal_part(cpy, 0, 0, len, NULL, mtx->main_diagonal);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(cpy);
            return res;
    ...
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        ...
        const int32_t new_idx = -(int32_t)idx;
        _Complex double const* dia_ptr = mtx->sub_diagonals.diagonals[i];
        res = jmtxz_matrix_cds_set_diagonal_full(cpy, new_idx, dia_ptr);
        if (res != JMTX_RESULT_SUCCESS)
        {
            jmtxz_matrix_cds_destroy(cpy);
            return res;
 ...
 */
void jmtxz_matrix_cds_set_diagonal(jmtxz_matrix_cds* mtx, int32_t dia_idx, _Complex double v)
{
    _Complex double* const ptr = jmtxz_matrix_cds_get_diagonal(mtx, dia_idx);
    if (ptr)
    {
        uint_fast32_t len;
        if (dia_idx < 0)
 ...
 * @param dia_idx diagonal to remove
 */
void jmtxz_matrix_cds_zero_diagonal(jmtxz_matrix_cds* mtx, int32_t dia_idx)
{
    _Complex double* const ptr = jmtxz_matrix_cds_get_diagonal(mtx, dia_idx);
    if (ptr)
    {
...
void jmtxz_matrix_cds_zero_row(const jmtxz_matrix_cds* mtx, uint32_t row)
{
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
...
void jmtxz_matrix_cds_zero_col(const jmtxz_matrix_cds* mtx, uint32_t col)
{
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        const uint_fast32_t dia = mtx->sub_diagonals.indices[i];
...
jmtx_result jmtxzs_matrix_cds_diagonal_count(const jmtxz_matrix_cds* mtx, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_diagonal_count(mtx);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_entries_in_dia(jmtxz_matrix_cds* mtx, int32_t dia, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_entries_in_dia(mtx, dia);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_set_diagonal_full(jmtxz_matrix_cds* mtx, int32_t dia_idx, const _Complex double values[])
{
    ...
    return jmtxz_matrix_cds_set_diagonal_full(mtx, dia_idx, values);
}
                                               ...
                                               uint32_t* p_count, const _Complex double values[JMTX_ARRAY_ATTRIB(static n)])
{
    ...
    return jmtxz_matrix_cds_set_diagonal_part(mtx, dia_idx, offset, n, p_count, values);
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxzs_matrix_cds_allocate_diagonal(jmtxz_matrix_cds* mtx, int32_t dia, uint32_t* p_size, _Complex double** p_dia)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    _Complex double* const ptr = jmtxz_matrix_cds_allocate_diagonal(mtx, dia, p_size);
    if (!ptr)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxzs_matrix_cds_allocate_zero_diagonal(jmtxz_matrix_cds* mtx, int32_t dia, uint32_t* p_size, _Complex double** p_dia)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    _Complex double* const ptr = jmtxz_matrix_cds_allocate_zero_diagonal(mtx, dia, p_size);
    if (!ptr)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
...
jmtx_result jmtxzs_matrix_cds_get_diagonal(const jmtxz_matrix_cds* mtx, int32_t dia, _Complex double** p_dia)
{
    ...
    *p_dia = jmtxz_matrix_cds_get_diagonal(mtx, dia);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_entries_in_row(const jmtxz_matrix_cds* mtx, uint32_t row, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_entries_in_row(mtx, row);
    return JMTX_RESULT_SUCCESS;
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxzs_matrix_cds_get_row(const jmtxz_matrix_cds* mtx, uint32_t row, uint32_t n, _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict n)],
                                     uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict n)], uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_get_row(mtx, row, n, p_values, p_cols);
    return JMTX_RESULT_SUCCESS;
}
                                     ...
                                     const _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict static n)], const uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    ...
    {
        if (!isfinite(creal(p_values[i])) || !isfinite(cimag(p_values[i])))
        {
            return JMTX_RESULT_BAD_PARAM;
        }
    }
    ...
    return jmtxz_matrix_cds_set_row(mtx, row, n, p_values, p_cols);
}
/**
 * Returns the number of entries in the column of the matrix
 * @param mtx pointer to the memory where the matrix is stored
...
jmtx_result jmtxzs_matrix_cds_entries_in_col(const jmtxz_matrix_cds* mtx, uint32_t col, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_entries_in_col(mtx, col);
    return JMTX_RESULT_SUCCESS;
}
 ...
 */
jmtx_result
jmtxzs_matrix_cds_get_col(const jmtxz_matrix_cds* mtx, uint32_t col, uint32_t n, _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict n)],
                         uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict n)], uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_get_col(mtx, col, n, p_values, p_rows);
    return JMTX_RESULT_SUCCESS;
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxzs_matrix_cds_vector_multiply(const jmtxz_matrix_cds* mtx, const _Complex double* restrict x, _Complex double* restrict y)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxz_matrix_cds_vector_multiply(mtx, x, y);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_set_entry(const jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    ...
    jmtxz_matrix_cds_set_entry(mtx, i, j, value);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_get_entry(const jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double* p_v)
{
    ...
    *p_v = jmtxz_matrix_cds_get_entry(mtx, i, j);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_insert_entry(jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    ...
    return jmtxz_matrix_cds_insert_entry(mtx, i, j, value);
}
...
jmtx_result jmtxzs_matrix_cds_add_to_entry(jmtxz_matrix_cds* mtx, uint32_t i, uint32_t j, _Complex double value)
{
    ...
    jmtxz_matrix_cds_add_to_entry(mtx, i, j, value);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_count_values(const jmtxz_matrix_cds* mtx, _Complex double v, uint32_t* p_count)
{
    ...
    *p_count = jmtxz_matrix_cds_count_values(mtx, v);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_zero_all_entries(const jmtxz_matrix_cds* mtx)
{
    ...
    jmtxz_matrix_cds_zero_all_entries(mtx);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_set_all_entries(const jmtxz_matrix_cds* mtx, _Complex double x)
{
    ...
    jmtxz_matrix_cds_set_all_entries(mtx, x);
    return JMTX_RESULT_SUCCESS;
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxzs_matrix_cds_transpose(
        const jmtxz_matrix_cds* mtx, jmtxz_matrix_cds** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!mtx)
    {
    ...
    return jmtxz_matrix_cds_transpose(mtx, p_out, allocator_callbacks);
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxzs_matrix_cds_copy(const jmtxz_matrix_cds* mtx, jmtxz_matrix_cds** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxz_matrix_cds_copy(mtx, p_out, allocator_callbacks);
}
...
jmtx_result jmtxzs_matrix_cds_set_diagonal(jmtxz_matrix_cds* mtx, int32_t dia_idx, _Complex double v)
{
    ...
    jmtxz_matrix_cds_set_diagonal(mtx, dia_idx, v);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxzs_matrix_cds_zero_diagonal(jmtxz_matrix_cds* mtx, int32_t dia_idx)
{
    ...
    jmtxz_matrix_cds_zero_diagonal(mtx, dia_idx);
    return JMTX_RESULT_SUCCESS;
}
                                     ...
                                     const _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict static n)], const uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    ...
    return jmtxz_matrix_cds_set_col(mtx, col, n, p_values, p_rows);
}
