...
static inline _Complex double *jmtxz_matrix_cds_diagonal_array_get_ptr(const jmtxz_matrix_cds_diagonal_array *this,
                                                                       uint32_t idx)
{
 ...
 */
jmtx_result jmtxz_matrix_cds_new(jmtxz_matrix_cds **p_mtx, uint32_t rows, uint32_t cols, uint32_t n_diagonals,
                                 ...
                                 const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    mtx_res =
        jmtxz_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->sub_diagonals,
    ...
    mtx_res =
        jmtxz_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->super_diagonals,
    ...
    for (uint32_t i = 0; i < n_diagonals; ++i)
    {
        if (p_dia_idx[i] < 0)
        {
            _Complex double *const ptr = allocator_callbacks->alloc(
    ...
    if (n_diagonals > n_sub + n_sup)
    {
        ...
        _Complex double *const main_ptr =
...
/**
 * Version of jmtxz_matrix_cds_set_row which does not touch the count of entries after the current row. This is useful
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_set_diagonal_full(jmtxz_matrix_cds *mtx, int32_t dia_idx,
                                               const _Complex double values[JMTX_ARRAY_ATTRIB(const)])
{
    uint32_t len;
...
_Complex double *jmtxz_matrix_cds_allocate_diagonal(jmtxz_matrix_cds *mtx, int32_t dia, uint32_t *p_size)
{
    ...
    else if (dia < 0)
    {
        ...
        if (!ptr)
        {
            ...
            const jmtx_result res =
                jmtxz_matrix_cds_diagonal_array_insert(&mtx->base.allocator_callbacks, &mtx->sub_diagonals, idx, ptr);
    ...
    else // if (dia_idx > 0)
    {
        ...
        if (!ptr)
        {
            ...
            const jmtx_result res =
                jmtxz_matrix_cds_diagonal_array_insert(&mtx->base.allocator_callbacks, &mtx->super_diagonals, idx, ptr);
            if (res != JMTX_RESULT_SUCCESS)
            {
                mtx->base.allocator_callbacks.free(mtx->base.allocator_callbacks.state, ptr);
                return NULL;
 ...
 * small)
 */
uint32_t jmtxz_matrix_cds_get_row(const jmtxz_matrix_cds *mtx, uint32_t row, uint32_t n,
 ...
 * small)
 */
uint32_t jmtxz_matrix_cds_get_col(const jmtxz_matrix_cds *mtx, uint32_t col, uint32_t n,
                                  _Complex double p_values[JMTX_ARRAY_ATTRIB(restrict n)],
 ...
 */
void jmtxz_matrix_cds_vector_multiply(const jmtxz_matrix_cds *mtx, const _Complex double *restrict x,
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_cds_transpose(const jmtxz_matrix_cds *mtx, jmtxz_matrix_cds **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
