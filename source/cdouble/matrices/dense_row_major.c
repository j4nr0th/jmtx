...
jmtx_result jmtxz_matrix_drm_transpose_inplace(jmtxz_matrix_drm* mtx, _Complex double* aux_row)
{
    ...
    if (mtx->permutations)
    {
        ...
        jmtxz_matrix_drm_commit_permutations2(mtx, aux_row);
    }
    ...
    for (uint32_t i = 0; i < (mtx->base.rows + 1) / 2; ++i)
    {
        for (uint32_t j = 0; j < mtx->base.cols; ++j)
        {
            const _Complex double tmp = mtx->values[j * mtx->base.cols + i];
...
void jmtxz_matrix_drm_commit_permutations(jmtxz_matrix_drm* mtx)
{
    ...
    for (pos = 0; pos < mtx->base.rows; ++pos)
    {
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            dst = mtx->rperm[pos];
            src = pos;
            _Complex double aux = mtx->values[n * src + i];
            while (mtx->rperm[src] != pos)
            {
                const _Complex double tmp1 = aux;
...
void jmtxz_matrix_drm_commit_permutations2(jmtxz_matrix_drm* mtx, _Complex double* aux_row)
{
    ...
    for (pos = 0; pos < mtx->base.rows; ++pos)
    {
        ...
        while (mtx->rperm[src] != pos)
        {
            for (uint32_t i = 0; i < n; ++i)
            {
                const _Complex double tmp = aux_row[i];
    ...
    jmtxz_matrix_drm* mtx, unsigned r1, unsigned r2, const _Complex double ct, const _Complex double st)
{
    ...
    {
        const _Complex double a = row1[j];
        const _Complex double b = row2[j];
        row1[j] = ct * a - st * b;
        row2[j] = st * a + ct * b;
    }
}
 ...
 _Complex double st)
{
    ...
    jmtxz_matrix_drm_givens_rotation_left(mtx, r1, r2, ct, st);
    return JMTX_RESULT_SUCCESS;
}
...
_Complex double st)
{
    ...
    if (!mtx->permutations)
    {
        ...
        {
            const _Complex double a = col1[i * mtx->base.cols];
            const _Complex double b = col2[i * mtx->base.cols];
    ...
    else
    {
        for (unsigned i = 0; i < n; ++i)
        {
            const _Complex double a = col1[mtx->permutations[i] * mtx->base.cols];
            const _Complex double b = col2[mtx->permutations[i] * mtx->base.cols];
...
_Complex double st)
{
    ...
    jmtxz_matrix_drm_givens_rotation_right(mtx, c1, c2, ct, st);
    return JMTX_RESULT_SUCCESS;
}
