// Automatically generated from source/float/solvers/gauss_seidel_iteration.c on Sun Dec 17 16:44:45 2023
//
 ...
 */
jmtx_result jmtxz_solve_iterative_gauss_seidel_crs(const jmtxz_matrix_crs *mtx, const _Complex double *restrict y,
                                                   _Complex double *restrict x, _Complex double *restrict aux_vec1,
                                                   jmtxd_solver_arguments *args)
{
    //  Length of x and y
    const uint32_t n = mtx->base.cols;
    _Complex double *const div_factors = aux_vec1;
    double mag_y = 0;
    ...
    for (;;)
    {
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            _Complex double res = jmtxz_matrix_crs_vector_multiply_row(mtx, x, i);
            const _Complex double new_x = (y[i] - res) * div_factors[i] + x[i];
            x[i] = new_x;
        }
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            const _Complex double val = y[i] - jmtxz_matrix_crs_vector_multiply_row(mtx, x, i);
            err += conj(val) * val;
        }
        err = sqrt(err) / mag_y;
 ...
 * in case of failure it returns the associated error code
 */
jmtx_result jmtxzs_solve_iterative_gauss_seidel_crs(const jmtxz_matrix_crs *mtx, uint32_t n,
                                                    ...
                                                    jmtxd_solver_arguments *args)
{
    ...
    return jmtxz_solve_iterative_gauss_seidel_crs(mtx, y, x, aux_vec1, args);
}
