// Automatically generated from source/cfloat/solvers/incomplete_lu_decomposition.c on Fri Dec  1 18:48:06 2023
// Automatically generated from source/cdouble/solvers/incomplete_lu_decomposition.c on Fri Dec  1 17:36:03 2023
//
        ...
        const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    {
        uint32_t n_dim = jmtxz_matrix_crs_entries_in_col(a, i);
        if (n_dim > max_elements_in_direction)
        {
            max_elements_in_direction = n_dim;
        }
        uint32_t* unused_idx;
        _Complex double* unused_val;
        n_dim = jmtxz_matrix_crs_get_row(a, i, &unused_idx, &unused_val);
        n_dim += 1;
        if (n_dim > max_elements_in_direction)
        {
            max_elements_in_direction = n_dim;
    ...
    _Complex double* p_values = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*p_values) * 2 * max_elements_in_direction);
    if (!p_values)
    {
    ...
    res = jmtxz_matrix_crs_new(&l, n, n, max_entries, allocator_callbacks);
    ...
    res = jmtxz_matrix_ccs_new(&u, n, n, max_entries, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        jmtxz_matrix_crs_destroy(l);
        allocator_callbacks->free(allocator_callbacks->state, p_values);
        allocator_callbacks->free(allocator_callbacks->state, p_indices);
        //  Can't make matrix :(
        return res;
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        ...
        //  Get a row from A
        c = jmtxz_matrix_crs_get_row(a, i, &indices, &values);
        uint32_t r;
        for (r = 0; r < c && indices[r] < i; ++r)
        {
            const uint32_t m = indices[r];
            _Complex double v = 0;
            _Complex double va = values[r];
            ...
            u_col_count = jmtxz_matrix_ccs_get_col(u, m, &u_row_indices, &u_val);
        ...
        res = jmtxz_matrix_crs_build_row(l, i, r + 1, p_indices, p_values);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, p_values);
            allocator_callbacks->free(allocator_callbacks->state, p_indices);
            jmtxz_matrix_ccs_destroy(u);
            jmtxz_matrix_crs_destroy(l);
            return res;
        }

        //  Get column values from the matrix A
        c = jmtxz_matrix_crs_get_col(a, i, max_elements_in_direction, p_values, p_indices);
        _Complex double* const p_vu = p_values + max_elements_in_direction;
...
//        const uint32_t r_before = r;
        for (r = 0; r < c && p_indices[r] <= i; ++r)
        {
            ...
            _Complex double v = 0;
            _Complex double va = p_values[r];
            assert(va != 0.0f);
            uint32_t* l_col_indices;
            _Complex double* l_val;
            uint32_t l_row_count = jmtxz_matrix_crs_get_row(l, m, &l_col_indices, &l_val);
            ...
            v = (va - v);
//                jmtxz_matrix_ccs_set_entry(u, m, p, v);
            p_vu[r] = v;
            p_iu[r] = m;
        }
        ...
        res = jmtxz_matrix_ccs_build_col(u, i, r, p_iu, p_vu);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, p_values);
            allocator_callbacks->free(allocator_callbacks->state, p_indices);
        ...
        const jmtx_allocator_callbacks* allocator_callbacks, jmtxd_solver_arguments* args)
{
    ...
    _Complex double* column_values = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*column_values) * max_elements_in_direction);
    if (!column_values)
    {
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        ...
        pos += 1;
        //  Values bellow the diagonal go to L
        res = jmtxz_matrix_crs_build_row(l, i, pos, column_indices, column_values);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, column_values);
            allocator_callbacks->free(allocator_callbacks->state, column_indices);
        ...
        //  Get column values from the matrix A
        c = jmtxz_matrix_crs_get_col(a, i, max_elements_in_direction, column_values, column_indices);
        ...
        //  Values above the diagonal go to U
        res = jmtxz_matrix_ccs_build_col(u, i, pos + 1, indices, values);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, column_values);
            allocator_callbacks->free(allocator_callbacks->state, column_indices);
...
max_iterations, allocator_callbacks, a, l, u, n, convergence, converged, any_failed_allocating, p_err)
    {
        ...
        _Complex double* const col_val = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*col_val) * max_elements_in_direction);
        if (!col_idx || !col_val)
        {
...
#pragma omp for schedule(static)
                for (uint32_t p = 0; p < n; ++p)
                {
                    ...
                    uint32_t* positions;
                    _Complex double* values;
                    n_items = jmtxz_matrix_crs_get_row(a, p, &positions, &values);
                    uint32_t out_items;
                    uint32_t* out_positions;
                    _Complex double* out_values;
                    out_items = jmtxz_matrix_crs_get_row(l, p, &out_positions, &out_values);
                    ...
                    for (uint32_t r = 0, k = 0; r < n_items && (int) positions[r] < (int) p; ++r, ++k)
                    {
                        ...
                        v = (va - v) / u_val[u_col_count - 1];
                        const double relative_change = cabs((v - out_values[k]) / out_values[k]);
                    ...
                    n_items =  jmtxz_matrix_crs_get_col(a, p, max_elements_in_direction, col_val, col_idx);
                    out_items = jmtxz_matrix_ccs_get_col(u, p, &out_positions, &out_values);
                    //  Compute the product of row m of matrix L and column p of matrix U to update the entry U_mp
                    for (uint32_t r = 0, k = 0; r < n_items && col_idx[r] < p + 1; ++r, ++k)
                    {
                        const uint32_t m = col_idx[r];
                        _Complex double v = 0;
                        _Complex double va = col_val[r];
                        uint32_t l_row_count;
                        uint32_t* l_col_indices;
                        _Complex double* l_val;
                        l_row_count = jmtxz_matrix_crs_get_row(l, m, &l_col_indices, &l_val);
        ...
        _Complex double* final_max_change, uint32_t* p_last_iteration, const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    while (iteration_count < max_iterations)
    {
        ...
        for (uint32_t p = 0; p < n; ++p)
        {
            ...
            uint32_t* positions;
            _Complex double* values;
            uint32_t n_items = jmtxz_matrix_crs_get_row(a, p, &positions, &values);
            uint32_t* out_positions;
            _Complex double* out_values;
            uint32_t out_items = jmtxz_matrix_crs_get_row(l, p, &out_positions, &out_values);
            //  Loop over elements of L which can be updated
            for (uint32_t r = 0, k = 0; r < n_items && (int)positions[r] < (int)p; ++r, ++k)
            {
                const uint32_t m = positions[r];
...
#ifndef NDEBUG
                _Complex double v1 = jmtxz_matrix_crs_get_entry(l, p, m);
                assert(v1 != 0.0f);
#endif
//                jmtxz_matrix_crs_set_entry(l, p, m, v);
                out_values[k] = v;
            }

            //  Update the p-th column of U
            n_items = jmtxz_matrix_crs_get_col(a, p, max_elements_in_direction, column_values, column_indices);
            out_items = jmtxz_matrix_ccs_get_col(u, p, &out_positions, &out_values);
            //  Compute the product of row m of matrix L and column p of matrix U to update the entry U_mp
            for (uint32_t r = 0, k = 0; r < n_items && column_indices[r] < p + 1; ++r, ++k)
            {
                ...
                assert(out_positions[k] == m);
                _Complex double v = 0;
                _Complex double va = column_values[r];
                assert(va != 0.0f);
                uint32_t* l_col_indices;
                _Complex double* l_val;
                assert(va != 0.0f);
...
#ifndef NDEBUG
                _Complex double v1 = jmtxz_matrix_ccs_get_entry(u, m, p);
                assert(v1 != 0.0f);
#endif
//                jmtxz_matrix_ccs_set_entry(u, m, p, v);
                out_values[k] = v;
