    ...
    _Complex double *restrict aux_vecs3, jmtxd_solver_arguments *args)
{
    ...
    _Complex double *const r = aux_vec6;
    ...
    _Complex double *const s_vectors = aux_vecs2;
    _Complex double *const p_vectors = aux_vecs3;
    //  Begin iterations
    for (;;)
    {
        _Complex double *const s = s_vectors + n * vec_idx;
        _Complex double *const p = p_vectors + n * vec_idx;
        for (uint32_t i = 0; i < n; ++i)
        {
            p[i] = 0;
        }
        //  Use GMRES(m) to generate a correction to the current error
        (void)jmtxz_gmresm_round_cds(mtx, n, m, y_mag, args->in_convergence_criterion, r, p, r_mtx, aux_vec1, aux_vec2,
                                     aux_vec3, aux_vec4, aux_vec5, aux_vecs1);
        //  Generate conjugate vector
        jmtxz_matrix_cds_vector_multiply(mtx, p, s);
        ...
        for (uint32_t i = 0; i < vec_cnt; ++i)
        {
            ...
            _Complex double dp = 0;
            const _Complex double *other_s = s_vectors + i * n;
            const _Complex double *other_p = p_vectors + i * n;
    ...
    _Complex double aux_vecs3[JMTX_ARRAY_ATTRIB(restrict l * n)], jmtxd_solver_arguments *args)
{
    ...
    if (r_mtx->base.type != JMTXZ_TYPE_BRM)
    {
        return JMTX_RESULT_WRONG_TYPE;
    }
    ...
    return jmtxz_solve_iterative_gmresr_cds(mtx, y, x, m, l, r_mtx, aux_vec1, aux_vec2, aux_vec3, aux_vec4, aux_vec5,
    ...
    _Complex double *restrict aux_vecs3, jmtxd_solver_arguments *args)
{
    ...
    for (;;)
    {
        ...
        //  Use GMRES(m) to generate a correction to the current error
        (void)jmtxz_gmresm_round_crs(mtx, n, m, y_mag, args->in_convergence_criterion, r, p, r_mtx, aux_vec1, aux_vec2,
                                     aux_vec3, aux_vec4, aux_vec5, aux_vecs1);
        //  Generate conjugate vector
        jmtxz_matrix_crs_vector_multiply(mtx, p, s);
    ...
    _Complex double aux_vecs3[JMTX_ARRAY_ATTRIB(restrict l * n)], jmtxd_solver_arguments *args)
{
    ...
    return jmtxz_solve_iterative_gmresr_crs(mtx, y, x, m, l, r_mtx, aux_vec1, aux_vec2, aux_vec3, aux_vec4, aux_vec5,
                                            aux_vec6, aux_vecs1, aux_vecs2, aux_vecs3, args);
}
