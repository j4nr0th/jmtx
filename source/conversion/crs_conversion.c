                                             ...
                                             const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    JMTX_NAME_OUT(matrix_crs) *mtx = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*mtx));
    if (!mtx)
    {
    ...
    mtx->base.type = JMTX_OUTPUT_ENUM(CRS);
...
JMTX_NAME_OUT(matrix_crs) * JMTX_NAME_CONVERSION(matrix_crs_inplace)(JMTX_NAME_IN(matrix_crs) * in)
{
    ...
    in->capacity = count;
    in->base.type = JMTX_OUTPUT_ENUM(CRS);

    return (JMTX_NAME_OUT(matrix_crs) *)in;
}
