        ...
        const jmtx_matrix_ccs* in, jmtx_matrix_crs** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    jmtx_matrix_crs* out;
    jmtx_result res = jmtx_matrix_crs_new(&out, rows, in->base.cols, in->n_entries, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
    ...
    for (uint32_t col = 0; col < in->base.cols; ++col)
    {
        uint32_t* in_rows;
        float* in_vals;
        uint32_t n_col = jmtx_matrix_ccs_get_col(in, col, &in_rows, &in_vals);
