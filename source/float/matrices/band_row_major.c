...
static inline uint_fast32_t brm_row_offset(const jmtxf_matrix_brm mtx[JMTX_ARRAY_ATTRIB(const static 1)],
...
static inline uint_fast32_t brm_row_len(const jmtxf_matrix_brm mtx[JMTX_ARRAY_ATTRIB(const static 1)],
                                        const uint_fast32_t row)
{
...
jmtx_result jmtxf_matrix_brm_new(jmtxf_matrix_brm **p_mtx, uint32_t rows, uint32_t cols, uint32_t ubw, uint32_t lbw,
                                 ...
                                 const float *set_value, const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxf_matrix_brm_new(p_mtx, rows, cols, ubw, lbw, set_value, allocator_callbacks);
}

void jmtxf_matrix_brm_destroy(jmtxf_matrix_brm *mtx)
{
...
jmtx_result jmtxs_matrix_brm_destroy(jmtxf_matrix_brm *mtx)
{
    ...
    jmtxf_matrix_brm_destroy(mtx);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_set_row(const jmtxf_matrix_brm *mtx, uint32_t row, const float values[])
{
...
jmtx_result jmtxs_matrix_brm_set_row(const jmtxf_matrix_brm *mtx, uint32_t row, const float values[])
{
    ...
    jmtxf_matrix_brm_set_row(mtx, row, values);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_vector_multiply(const jmtxf_matrix_brm *mtx, const float *restrict x, float *restrict y)
{
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        const float *const values = mtx->values + brm_row_offset(mtx, i);
        const uint_fast32_t first_elm = jmtxf_matrix_brm_first_pos_in_row(mtx, i);
        const uint_fast32_t last_elm = jmtxf_matrix_brm_last_pos_in_row(mtx, i);
...
jmtx_result jmtxs_matrix_brm_vector_multiply(const jmtxf_matrix_brm *mtx, const float *restrict x, float *restrict y)
{
    ...
    jmtxf_matrix_brm_vector_multiply(mtx, x, y);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_set_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j, float value)
{
    float *const values = mtx->values + brm_row_offset(mtx, i);
    const uint_fast32_t offset = j - jmtxf_matrix_brm_first_pos_in_row(mtx, i);
    assert(j >= jmtxf_matrix_brm_first_pos_in_row(mtx, i));
    assert(j <= jmtxf_matrix_brm_last_pos_in_row(mtx, i));
    values[offset] = value;
}
...
jmtx_result jmtxs_matrix_brm_set_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j, float value)
{
    ...
    if (j < jmtxf_matrix_brm_first_pos_in_row(mtx, i))
    {
        return JMTX_RESULT_INDEX_OUT_OF_BOUNDS;
    }
    if (j > jmtxf_matrix_brm_last_pos_in_row(mtx, i))
    {
        return JMTX_RESULT_INDEX_OUT_OF_BOUNDS;
    }
    jmtxf_matrix_brm_set_entry(mtx, i, j, value);
    return JMTX_RESULT_SUCCESS;
}

float jmtxf_matrix_brm_get_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j)
{
    if ((j < jmtxf_matrix_brm_first_pos_in_row(mtx, i)) || (j > jmtxf_matrix_brm_last_pos_in_row(mtx, i)))
    {
        return 0.0f;
    }
    float *const values = mtx->values + brm_row_offset(mtx, i);
...
jmtx_result jmtxs_matrix_brm_get_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j, float *p_value)
{
    ...
    *p_value = jmtxf_matrix_brm_get_entry(mtx, i, j);

    return JMTX_RESULT_SUCCESS;
}

uint_fast32_t jmtxf_matrix_brm_get_row(const jmtxf_matrix_brm *mtx, uint32_t row, float *p_elements[1])
{
...
jmtx_result jmtxs_matrix_brm_get_row(const jmtxf_matrix_brm *mtx, uint_fast32_t *n, uint32_t row, float *p_elements[1])
{
    ...
    *n = jmtxf_matrix_brm_get_row(mtx, row, p_elements);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_brm_count_values(const jmtxf_matrix_brm *mtx, float v)
{
    uint32_t r = 0;
...
jmtx_result jmtxs_matrix_brm_count_values(const jmtxf_matrix_brm *mtx, float v, uint32_t *p_count)
{
    ...
    *p_count = jmtxf_matrix_brm_count_values(mtx, v);
    return JMTX_RESULT_SUCCESS;
}

// static inline uint_fast32_t entries_above_diagonal(const jmtxf_matrix_brm mtx[JMTX_ARRAY_ATTRIB(const static 1)],
// uint_fast32_t col)
//{
...
// }
//
// static inline uint_fast32_t entries_bellow_diagonal(const jmtxf_matrix_brm mtx[JMTX_ARRAY_ATTRIB(const static 1)],
// uint_fast32_t col)
//{
...
uint32_t jmtxf_matrix_brm_length_of_col(const jmtxf_matrix_brm *mtx, uint32_t col)
{
    return jmtxf_matrix_brm_last_pos_in_col(mtx, col) - jmtxf_matrix_brm_first_pos_in_col(mtx, col) + 1;
}
...
jmtx_result jmtxs_matrix_brm_length_of_col(const jmtxf_matrix_brm *mtx, uint32_t col, uint32_t *p_n)
{
    ...
    *p_n = jmtxf_matrix_brm_length_of_col(mtx, col);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_brm_get_col(const jmtxf_matrix_brm *mtx, uint32_t col, float values[])
{
...
#endif
    pos_rel = (col - jmtxf_matrix_brm_first_pos_in_row(mtx, first_row));
    i = brm_row_offset(mtx, first_row) + pos_rel;
    for (row = first_row, j = 0; row < last_row; ++row)
    {
        assert(i < max);
        values[j++] = mtx->values[i];
        uint_fast32_t new_pos_rel = (col - jmtxf_matrix_brm_first_pos_in_row(mtx, row + 1));
...
jmtx_result jmtxs_matrix_brm_get_col(const jmtxf_matrix_brm *mtx, uint32_t col, uint32_t *p_count, float p_values[])
{
    ...
    *p_count = jmtxf_matrix_brm_get_col(mtx, col, p_values);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_brm_transpose(const jmtxf_matrix_brm *mtx, jmtxf_matrix_brm **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtxf_matrix_brm *const out = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*out));
    if (!out)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
    ...
    {
        n += jmtxf_matrix_brm_get_col(mtx, j, new_values + n);
    }
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxf_matrix_brm_transpose(mtx, p_out, allocator_callbacks);
}

jmtx_result jmtxf_matrix_brm_copy(const jmtxf_matrix_brm *mtx, jmtxf_matrix_brm **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
                                  ...
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxf_matrix_brm_copy(mtx, p_out, allocator_callbacks);
}

float jmtxf_matrix_brm_vector_multiply_row(const jmtxf_matrix_brm *mtx, const float *x, uint32_t i)
{
    const uint_fast32_t offset = brm_row_offset(mtx, i);
    const uint_fast32_t first_col = jmtxf_matrix_brm_first_pos_in_row(mtx, i);
                                                 ...
                                                 float *restrict p_r)
{
    ...
    *p_r = jmtxf_matrix_brm_vector_multiply_row(mtx, x, i);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_add_to_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j, float value)
{
    uint_fast32_t row_offset = brm_row_offset(mtx, i);
    uint_fast32_t col_offset = j - jmtxf_matrix_brm_first_pos_in_row(mtx, i);
...
jmtx_result jmtxs_matrix_brm_add_to_entry(const jmtxf_matrix_brm *mtx, uint32_t i, uint32_t j, float value)
{
    ...
    jmtxf_matrix_brm_add_to_entry(mtx, i, j, value);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_zero_all_entries(const jmtxf_matrix_brm *mtx)
{
...
jmtx_result jmtxs_matrix_brm_zero_all_entries(const jmtxf_matrix_brm *mtx)
{
    ...
    jmtxf_matrix_brm_zero_all_entries(mtx);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_brm_set_all_entries(const jmtxf_matrix_brm *mtx, float x)
{
...
JMTX_HOT_FUNCTION
uint_fast32_t jmtxf_matrix_brm_first_pos_in_row(const jmtxf_matrix_brm *mtx, uint32_t row)
{
    if (row < mtx->lower_bandwidth)
    {
        return 0;
...
uint_fast32_t jmtxf_matrix_brm_last_pos_in_row(const jmtxf_matrix_brm *mtx, uint32_t row)
{
    if (row > mtx->base.rows - 1 - mtx->upper_bandwidth)
    {
...
uint_fast32_t jmtxf_matrix_brm_first_pos_in_col(const jmtxf_matrix_brm *mtx, uint32_t col)
{
    if (col < mtx->upper_bandwidth)
    {
        return 0;
...
uint_fast32_t jmtxf_matrix_brm_last_pos_in_col(const jmtxf_matrix_brm *mtx, uint32_t col)
{
...
uint_fast32_t jmtxf_matrix_brm_length_of_row(const jmtxf_matrix_brm *mtx, uint32_t row)
{
    return brm_row_len(mtx, row);
}

void jmtxf_matrix_brm_set_col(const jmtxf_matrix_brm *mtx, uint32_t col, const float values[])
{
    uint_fast32_t first_row;
    if (col < mtx->upper_bandwidth)
    {
...
void jmtxf_matrix_brm_get_bandwidths(const jmtxf_matrix_brm *mtx, uint32_t *ubw, uint32_t *lbw)
{
...
jmtx_result jmtxs_matrix_brm_set_all_entries(const jmtxf_matrix_brm *mtx, float x)
{
    ...
    jmtxf_matrix_brm_set_all_entries(mtx, x);
...
jmtx_result jmtxs_matrix_brm_set_col(const jmtxf_matrix_brm *mtx, uint32_t col, const float values[])
{
    ...
    const uint_fast32_t len = jmtxf_matrix_brm_length_of_col(mtx, col);
    for (uint32_t i = 0; i < len; ++i)
    {
        if (!isfinite(values[i]))
        {
    ...
    jmtxf_matrix_brm_set_col(mtx, col, values);
    return JMTX_RESULT_SUCCESS;
}
