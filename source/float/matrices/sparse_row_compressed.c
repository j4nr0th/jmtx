...
static uint32_t crs_get_row_entries(const jmtxf_matrix_crs *mtx, uint32_t row, uint32_t *pp_indices[1],
                                    float *pp_values[1])
{
    uint32_t offset, len;
    if (row == 0)
 ...
 */
static jmtx_result crs_insert_entry_at(jmtxf_matrix_crs *mtx, uint32_t row, uint32_t position, float value,
                                       uint32_t index)
{
...
jmtx_result jmtxf_matrix_crs_new(jmtxf_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
...
jmtx_result jmtxs_matrix_crs_new(jmtxf_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
                                 const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!p_mtx)
    {
    ...
    return jmtxf_matrix_crs_new(p_mtx, rows, cols, reserved_entries, allocator_callbacks);
}

void jmtxf_matrix_crs_destroy(jmtxf_matrix_crs *mtx)
{
...
jmtx_result jmtxs_matrix_crs_destroy(jmtxf_matrix_crs *mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_destroy(mtx);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_shrink(jmtxf_matrix_crs *mtx)
{
...
jmtx_result jmtxs_matrix_crs_shrink(jmtxf_matrix_crs *mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxf_matrix_crs_shrink(mtx);
}

jmtx_result jmtxf_matrix_crs_set_row(jmtxf_matrix_crs *mtx, uint32_t row, uint32_t n,
                                     const uint32_t indices[JMTX_ARRAY_ATTRIB(static n)],
                                     const float values[JMTX_ARRAY_ATTRIB(static n)])
{
...
jmtx_result jmtxs_matrix_crs_set_row(jmtxf_matrix_crs *mtx, uint32_t row, uint32_t n, const uint32_t *indices,
                                     const float *values)
{
    if (!mtx)
    {
    ...
    return jmtxf_matrix_crs_set_row(mtx, row, n, indices, values);
}

void jmtxf_matrix_crs_vector_multiply(const jmtxf_matrix_crs *mtx, const float *restrict x, float *restrict y)
{
...
jmtx_result jmtxs_matrix_crs_vector_multiply(const jmtxf_matrix_crs *mtx, const float *restrict x, float *restrict y)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_vector_multiply(mtx, x, y);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_set_entry(jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j, float value)
{
    jmtx_result res;
...
jmtx_result jmtxs_matrix_crs_set_entry(jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j, float value)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxf_matrix_crs_set_entry(mtx, i, j, value);
}

float jmtxf_matrix_crs_get_entry(const jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j)
{
    uint32_t *row_indices;
    float *row_values;
    const uint32_t n_row_elements = crs_get_row_entries(mtx, i, &row_indices, &row_values);
...
jmtx_result jmtxs_matrix_crs_get_entry(const jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j, float *p_value)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_value = jmtxf_matrix_crs_get_entry(mtx, i, j);

    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_crs_get_row(const jmtxf_matrix_crs *mtx, uint32_t row, uint32_t *p_indices[1],
                                  float *p_elements[1])
{
    return crs_get_row_entries(mtx, row, p_indices, p_elements);
}

jmtx_result jmtxs_matrix_crs_get_row(const jmtxf_matrix_crs *mtx, uint32_t row, uint32_t *n, uint32_t **p_indices,
                                     float **p_elements)
{
    if (!mtx)
    {
    ...
    *n = jmtxf_matrix_crs_get_row(mtx, row, p_indices, p_elements);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_crs_count_values(const jmtxf_matrix_crs *mtx, float v)
{
    uint32_t r = 0;
...
jmtx_result jmtxs_matrix_crs_count_values(const jmtxf_matrix_crs *mtx, float v, uint32_t *p_count)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_count = jmtxf_matrix_crs_count_values(mtx, v);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_crs_count_indices(const jmtxf_matrix_crs *mtx, uint32_t v)
{
    uint32_t r = 0;
    for (uint32_t i = 0; i < mtx->n_entries; ++i)
    {
...
jmtx_result jmtxs_matrix_crs_count_indices(const jmtxf_matrix_crs *mtx, uint32_t v, uint32_t *p_count)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_count = jmtxf_matrix_crs_count_indices(mtx, v);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_apply_unary_fn(const jmtxf_matrix_crs *mtx,
                                            int (*unary_fn)(uint32_t i, uint32_t j, float *p_value, void *param),
                                            void *param)
{
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
...
jmtx_result jmtxs_matrix_crs_apply_unary_fn(const jmtxf_matrix_crs *mtx,
                                            int (*unary_fn)(uint32_t i, uint32_t j, float *p_value, void *param),
                                            void *param)
{
    if (!mtx)
    ...
    return jmtxf_matrix_crs_apply_unary_fn(mtx, unary_fn, param);
}

void jmtxf_matrix_crs_remove_zeros(jmtxf_matrix_crs *mtx)
{
    //  Update offsets
    uint32_t p, c = 0, r = 0;
...
jmtx_result jmtxs_matrix_crs_remove_zeros(jmtxf_matrix_crs *mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_remove_zeros(mtx);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_crs_remove_bellow_magnitude(jmtxf_matrix_crs *mtx, float v)
{
    //  Update offsets
    uint32_t p, c = 0, r = 0;
    while (mtx->end_of_row_offsets[r] == 0)
...
jmtx_result jmtxs_matrix_crs_remove_bellow_magnitude(jmtxf_matrix_crs *mtx, float v)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_remove_bellow_magnitude(mtx, v);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_crs_entries_in_col(const jmtxf_matrix_crs *mtx, uint32_t col)
{
    uint32_t element_count = 0;
    for (uint32_t row = 0; row < mtx->base.rows && (!row || (mtx->end_of_row_offsets[row - 1] != mtx->n_entries));
         ++row)
    {
        uint32_t *row_indices;
...
jmtx_result jmtxs_matrix_crs_entries_in_col(const jmtxf_matrix_crs *mtx, uint32_t col, uint32_t *p_n)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_n = jmtxf_matrix_crs_entries_in_col(mtx, col);
    return JMTX_RESULT_SUCCESS;
}

uint32_t jmtxf_matrix_crs_get_col(const jmtxf_matrix_crs *mtx, uint32_t col, uint32_t n,
                                  float p_values[JMTX_ARRAY_ATTRIB(n)], uint32_t p_rows[JMTX_ARRAY_ATTRIB(n)])
{
    uint32_t k = 0;
    for (uint32_t row = 0;
...
jmtx_result jmtxs_matrix_crs_get_col(const jmtxf_matrix_crs *mtx, uint32_t col, uint32_t n, uint32_t *p_count,
                                     float *p_values, uint32_t *p_rows)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    *p_count = jmtxf_matrix_crs_get_col(mtx, col, n, p_values, p_rows);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_transpose(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtxf_matrix_crs *out;
    jmtx_result res = jmtxf_matrix_crs_new(&out, mtx->base.cols, mtx->base.rows, mtx->n_entries, allocator_callbacks);
...
jmtx_result jmtxs_matrix_crs_transpose(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
    ...
    return jmtxf_matrix_crs_transpose(mtx, p_out, allocator_callbacks);
}

jmtx_result jmtxf_matrix_crs_copy(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
    ...
    jmtxf_matrix_crs *const out = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*out));
    if (!out)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
...
jmtx_result jmtxs_matrix_crs_copy(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
    ...
    return jmtxf_matrix_crs_copy(mtx, p_out, allocator_callbacks);
}

jmtx_result jmtxf_matrix_crs_build_row(jmtxf_matrix_crs *mtx, uint32_t row, uint32_t n,
                                       const uint32_t indices[JMTX_ARRAY_ATTRIB(static n)],
                                       const float values[JMTX_ARRAY_ATTRIB(static n)])
{
    jmtx_result res = JMTX_RESULT_SUCCESS;
...
float jmtxf_matrix_crs_vector_multiply_row(const jmtxf_matrix_crs *mtx, const float *x, uint32_t i)
{
    uint32_t *indices;
    float *values;
...
jmtx_result jmtxs_matrix_crs_vector_multiply_row(const jmtxf_matrix_crs *mtx, const float *restrict x, uint32_t i,
                                                 float *restrict p_r)
{
    if (!mtx)
    {
    ...
    *p_r = jmtxf_matrix_crs_vector_multiply_row(mtx, x, i);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_add_to_entry(jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j, float value)
{
    jmtx_result res;
    uint32_t *row_indices;
    float *row_values;
...
jmtx_result jmtxs_matrix_crs_add_to_entry(jmtxf_matrix_crs *mtx, uint32_t i, uint32_t j, float value)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    return jmtxf_matrix_crs_add_to_entry(mtx, i, j, value);
}

void jmtxf_matrix_crs_zero_all_entries(const jmtxf_matrix_crs *mtx)
{
    memset(mtx->values, 0, sizeof(*mtx->values) * mtx->n_entries);
}

jmtx_result jmtxs_matrix_crs_zero_all_entries(const jmtxf_matrix_crs *mtx)
{
    ...
    jmtxf_matrix_crs_zero_all_entries(mtx);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_crs_set_all_entries(const jmtxf_matrix_crs *mtx, float x)
{
...
jmtx_result jmtxs_matrix_crs_set_all_entries(jmtxf_matrix_crs *mtx, float x)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_set_all_entries(mtx, x);

    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_crs_remove_row(jmtxf_matrix_crs *mtx, uint32_t row)
{
    uint32_t *row_indices;
    float *row_values;
...
jmtx_result jmtxs_matrix_crs_remove_row(jmtxf_matrix_crs *mtx, uint32_t row)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_remove_row(mtx, row);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_crs_remove_column(jmtxf_matrix_crs *mtx, uint32_t col)
{
...
jmtx_result jmtxs_matrix_crs_remove_column(jmtxf_matrix_crs *mtx, uint32_t col)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_remove_column(mtx, col);
    return JMTX_RESULT_SUCCESS;
}

void jmtxf_matrix_crs_clear(jmtxf_matrix_crs *mtx)
{
    mtx->n_entries = 0;
    memset(mtx->end_of_row_offsets, 0, sizeof(*mtx->end_of_row_offsets) * mtx->base.rows);
}

jmtx_result jmtxs_matrix_crs_clear(jmtxf_matrix_crs *mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxf_matrix_crs_clear(mtx);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxf_matrix_crs_join_vertically(jmtxf_matrix_crs **output,
                                             const jmtx_allocator_callbacks *allocator_callbacks, unsigned k,
                                             const jmtxf_matrix_crs *matrix_list[JMTX_ARRAY_ATTRIB(static k)])
{
    ...
    {
        const jmtxf_matrix_crs *const e = matrix_list[i];
    ...
    jmtxf_matrix_crs *out;
    jmtx_result res = jmtxf_matrix_crs_new(&out, n_rows, col_count, element_count, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
    ...
    {
        const jmtxf_matrix_crs *const mtx = matrix_list[i];
        unsigned j;
        for (j = 0; j < mtx->base.rows; ++j)
        {
            ...
            const jmtx_result r1 = jmtxf_matrix_crs_build_row(out, pos + j, n, indices, values);
...
jmtx_result jmtxf_matrix_crs_new_like(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
...
jmtx_result jmtxs_matrix_crs_new_like(const jmtxf_matrix_crs *mtx, jmtxf_matrix_crs **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks, const float *p_val)
{
    if (!mtx)
    {
    ...
    return jmtxf_matrix_crs_new_like(mtx, p_out, allocator_callbacks, p_val);
}

uint32_t jmtxf_matrix_crs_find_upper_bandwidth(const jmtxf_matrix_crs *mtx)
{
 ...
 */
uint32_t jmtxf_matrix_crs_find_lower_bandwidth(const jmtxf_matrix_crs *mtx)
{
    //  Find the greatest distance above the main diagonal
    uint_fast32_t v_max = 0;
    for (uint_fast32_t i = 0, p = 0; i < mtx->base.rows; ++i)
...
jmtx_result jmtxs_matrix_crs_join_vertically(jmtxf_matrix_crs **output,
                                             const jmtx_allocator_callbacks *allocator_callbacks, unsigned int k,
                                             const jmtxf_matrix_crs **matrix_list)
{
    if (!output)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    const uint32_t col_count = matrix_list[0]->base.cols;
    for (const jmtxf_matrix_crs **pmtx = matrix_list; pmtx != matrix_list + k; ++pmtx)
    {
        const jmtxf_matrix_crs *mtx = *pmtx;
        if (!mtx)
        {
            return JMTX_RESULT_NULL_PARAM;
        }
    ...
    return jmtxf_matrix_crs_join_vertically(output, allocator_callbacks, k, matrix_list);
}
