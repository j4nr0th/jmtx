 ...
 */
jmtx_result jmtx_matrix_cds_new(jmtx_matrix_cds **p_mtx, uint32_t rows, uint32_t cols, uint32_t n_diagonals,
                                ...
                                const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    mtx_res =
        jmtx_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->sub_diagonals,
    ...
    mtx_res =
        jmtx_matrix_cds_diagonal_array_init(allocator_callbacks, &mtx->super_diagonals,
    ...
    for (uint32_t i = 0; i < n_diagonals; ++i)
    {
        if (p_dia_idx[i] < 0)
        {
            float *const ptr = allocator_callbacks->alloc(
...
/**
 * Version of jmtx_matrix_cds_set_row which does not touch the count of entries after the current row. This is useful
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtx_matrix_cds_set_diagonal_full(jmtx_matrix_cds *mtx, int32_t dia_idx,
                                              const float values[JMTX_ARRAY_ATTRIB(const)])
{
    uint32_t len;
...
jmtx_result jmtx_matrix_cds_set_diagonal_part(jmtx_matrix_cds *mtx, int32_t dia_idx, uint32_t offset, uint32_t n,
                                              uint32_t *p_count, const float values[JMTX_ARRAY_ATTRIB(static n)])
{
    uint32_t len;
    float *const ptr = jmtx_matrix_cds_allocate_diagonal(mtx, dia_idx, &len);
    if (!ptr)
...
float *jmtx_matrix_cds_allocate_diagonal(jmtx_matrix_cds *mtx, int32_t dia, uint32_t *p_size)
{
    ...
    else if (dia < 0)
    {
        ...
        if (!ptr)
        {
            ...
            const jmtx_result res =
                jmtx_matrix_cds_diagonal_array_insert(&mtx->base.allocator_callbacks, &mtx->sub_diagonals, idx, ptr);
    ...
    else // if (dia_idx > 0)
    {
        ...
        if (!ptr)
        {
            ...
            const jmtx_result res =
                jmtx_matrix_cds_diagonal_array_insert(&mtx->base.allocator_callbacks, &mtx->super_diagonals, idx, ptr);
            if (res != JMTX_RESULT_SUCCESS)
            {
                mtx->base.allocator_callbacks.free(mtx->base.allocator_callbacks.state, ptr);
                return NULL;
 ...
 * @return pointer to the newly allocated diagonal, or NULL in case it failed to do so
 */
float *jmtx_matrix_cds_allocate_zero_diagonal(jmtx_matrix_cds *mtx, int32_t dia, uint32_t *p_size)
{
    float *ptr;
    uint_fast32_t len;
    uint_fast32_t idx;
 ...
 * small)
 */
uint32_t jmtx_matrix_cds_get_row(const jmtx_matrix_cds *mtx, uint32_t row, uint32_t n,
                                 float p_values[JMTX_ARRAY_ATTRIB(restrict n)],
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtx_matrix_cds_set_row(jmtx_matrix_cds *mtx, uint32_t row, uint32_t n,
                                    const float p_values[JMTX_ARRAY_ATTRIB(restrict static n)],
                                    const uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    jmtx_matrix_cds_zero_row(mtx, row);
    for (uint_fast32_t m = 0; m < n; ++m)
    {
        const int32_t dia = (int32_t)p_cols[m] - (int32_t)row;
        float *ptr;
        float v;
        ptr = jmtx_matrix_cds_allocate_zero_diagonal(mtx, dia, NULL);
        if (!ptr)
        {
            return JMTX_RESULT_BAD_ALLOC;
        }
 ...
 */
jmtx_result jmtx_matrix_cds_set_col(jmtx_matrix_cds *mtx, uint32_t col, uint32_t n,
                                    const float p_values[JMTX_ARRAY_ATTRIB(restrict static n)],
                                    const uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    jmtx_matrix_cds_zero_col(mtx, col);
    for (uint_fast32_t m = 0; m < n; ++m)
    {
 ...
 * small)
 */
uint32_t jmtx_matrix_cds_get_col(const jmtx_matrix_cds *mtx, uint32_t col, uint32_t n,
                                 float p_values[JMTX_ARRAY_ATTRIB(restrict n)],
...
/**
 * Similar to jmtx_matrix_cds_zero_all_entries, but slower, since it can not use memset. On the other hand, it allows
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtx_matrix_cds_transpose(const jmtx_matrix_cds *mtx, jmtx_matrix_cds **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
    ...
    jmtx_result res = jmtx_matrix_cds_new(&tps, new_rows, new_cols, dia_count, p_dia, allocator_callbacks);
                                 ...
                                 const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtx_matrix_cds_new(&cpy, new_rows, new_cols, dia_count, p_dia, allocator_callbacks);
    allocator_callbacks->free(allocator_callbacks->state, p_dia);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
...
void jmtx_matrix_cds_zero_row(const jmtx_matrix_cds *mtx, uint32_t row)
{
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
...
void jmtx_matrix_cds_zero_col(const jmtx_matrix_cds *mtx, uint32_t col)
{
    for (uint_fast32_t i = 0; i < mtx->sub_diagonals.count; ++i)
    {
        const uint_fast32_t dia = mtx->sub_diagonals.indices[i];
 ...
 */
jmtx_result jmtxs_matrix_cds_new(jmtx_matrix_cds **p_mtx, uint32_t rows, uint32_t cols, uint32_t n_diagonals,
                                 const int32_t p_dia_idx[JMTX_ARRAY_ATTRIB(static n_diagonals)],
                                 const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!p_mtx)
    ...
    return jmtx_matrix_cds_new(p_mtx, rows, cols, n_diagonals, p_dia_idx, allocator_callbacks);
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_allocate_zero_diagonal(jmtx_matrix_cds *mtx, int32_t dia, uint32_t *p_size, float **p_dia)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    float *const ptr = jmtx_matrix_cds_allocate_zero_diagonal(mtx, dia, p_size);
    if (!ptr)
    {
        return JMTX_RESULT_BAD_ALLOC;
    }
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_zero_row(const jmtx_matrix_cds *mtx, uint32_t row)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_zero_col(const jmtx_matrix_cds *mtx, uint32_t col)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_get_row(const jmtx_matrix_cds *mtx, uint32_t row, uint32_t n,
                                     float p_values[JMTX_ARRAY_ATTRIB(restrict n)],
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_set_row(jmtx_matrix_cds *mtx, uint32_t row, uint32_t n,
                                     const float p_values[JMTX_ARRAY_ATTRIB(restrict static n)],
                                     const uint32_t p_cols[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    if (!mtx)
    ...
    return jmtx_matrix_cds_set_row(mtx, row, n, p_values, p_cols);
}
/**
 * Returns the number of entries in the column of the matrix
 * @param mtx pointer to the memory where the matrix is stored
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_get_col(const jmtx_matrix_cds *mtx, uint32_t col, uint32_t n,
                                     float p_values[JMTX_ARRAY_ATTRIB(restrict n)],
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_transpose(const jmtx_matrix_cds *mtx, jmtx_matrix_cds **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxs_matrix_cds_copy(const jmtx_matrix_cds *mtx, jmtx_matrix_cds **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
 ...
 * @return JMTX_RESULT_SUCCESS if successful
 */
jmtx_result jmtxs_matrix_cds_set_col(jmtx_matrix_cds *mtx, uint32_t col, uint32_t n,
                                     const float p_values[JMTX_ARRAY_ATTRIB(restrict static n)],
                                     const uint32_t p_rows[JMTX_ARRAY_ATTRIB(restrict static n)])
{
    if (!mtx)
    ...
    return jmtx_matrix_cds_set_col(mtx, col, n, p_values, p_rows);
}
