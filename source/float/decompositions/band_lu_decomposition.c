                                   ...
                                   const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtxf_matrix_brm *l = NULL;
    jmtxf_matrix_brm *u = NULL;
    ...
    jmtx_result res = jmtxf_matrix_brm_new(&l, n, n, 0, lbw, NULL, allocator_callbacks);
    ...
    res = jmtxf_matrix_brm_new(&u, n, n, ubw, 0, NULL, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        jmtxf_matrix_brm_destroy(l);
        allocator_callbacks->free(allocator_callbacks->state, p_values);
        //  Can't make matrix :(
        return res;
    }
    ...
    {
        uint_fast32_t first_l = jmtxf_matrix_brm_first_pos_in_row(l, i);
        float *lwr_elements = NULL;
        jmtxf_matrix_brm_get_row(l, i, &lwr_elements);
        uint_fast32_t k = 0;
        float *a_elements;
        (void)jmtxf_matrix_brm_get_row(a, i, &a_elements);
        float *const upr_elements = p_values + max_entries;
        for (uint_fast32_t pl = first_l; pl < i; ++pl)
        {
            const uint_fast32_t count_upr = jmtxf_matrix_brm_get_col(u, pl, upr_elements);
            float v = 0;
            uint_fast32_t first_u = jmtxf_matrix_brm_first_pos_in_col(u, pl);
        ...
        const uint_fast32_t first_u = jmtxf_matrix_brm_first_pos_in_col(u, i);
        k = 0;
        (void)jmtxf_matrix_brm_get_col(a, i, p_values);
        a_elements = p_values;
        for (uint_fast32_t pu = first_u; pu <= i; ++pu)
        {
            jmtxf_matrix_brm_get_row(l, pu, &lwr_elements);
            float v = 0;
            first_l = jmtxf_matrix_brm_first_pos_in_row(l, pu);
        ...
        jmtxf_matrix_brm_set_col(u, i, upr_elements);
    }
