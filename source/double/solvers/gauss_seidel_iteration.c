// Automatically generated from source/float/solvers/gauss_seidel_iteration.c on Sun Dec 17 16:44:34 2023
//
 ...
 */
jmtx_result jmtxd_solve_iterative_gauss_seidel_crs(const jmtxd_matrix_crs *mtx, const double *restrict y,
                                                   double *restrict x, double *restrict aux_vec1,
                                                   jmtxd_solver_arguments *args)
{
    //  Length of x and y
    const uint32_t n = mtx->base.cols;
    double *const div_factors = aux_vec1;
    double mag_y = 0;
    ...
    for (;;)
    {
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            double res = jmtxd_matrix_crs_vector_multiply_row(mtx, x, i);
            const double new_x = (y[i] - res) * div_factors[i] + x[i];
            x[i] = new_x;
        }
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            const double val = y[i] - jmtxd_matrix_crs_vector_multiply_row(mtx, x, i);
            err += val * val;
        }
        err = sqrt(err) / mag_y;
 ...
 * in case of failure it returns the associated error code
 */
jmtx_result jmtxds_solve_iterative_gauss_seidel_crs(const jmtxd_matrix_crs *mtx, uint32_t n,
