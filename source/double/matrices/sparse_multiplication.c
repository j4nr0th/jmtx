                                      ...
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxd_matrix_crs_new(&out, r_out, c_out, a->n_entries + b->n_entries, allocator_callbacks);
        ...
        for (uint32_t j = 0; j < c_out; ++j)
        {
            ...
            if (v != 0)
            {
                if (count == capacity)
                {
                    ...
                    double *const new_v = allocator_callbacks->realloc(allocator_callbacks->state, values,
                                      ...
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxd_matrix_ccs_new(&out, r_out, c_out, a->n_entries + b->n_entries, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
...
double jmtxd_multiply_matrix_sparse_vectors(uint32_t n_a, const uint32_t i_a[JMTX_ARRAY_ATTRIB(const static n_a)],
                                            const double v_a[JMTX_ARRAY_ATTRIB(const static n_a)], uint32_t n_b,
                                            const uint32_t i_b[JMTX_ARRAY_ATTRIB(const static n_b)],
                                            const double v_b[JMTX_ARRAY_ATTRIB(const static n_b)])
{
    double v = 0;
    uint32_t ia = 0, ib = 0;
    while (ia < n_a && ib < n_b)
...
double jmtxd_multiply_matrix_sparse_vectors_limit(uint32_t max_a, uint32_t max_b, uint32_t n_a,
                                                  ...
                                                  const double v_a[JMTX_ARRAY_ATTRIB(static max_a)], uint32_t n_b,
                                      ...
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxd_matrix_brm_new(&out, r_out, c_out, max_ubw, max_lbw, NULL, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
    ...
    for (uint32_t i = 0; i < r_out; ++i)
    {
        ...
        for (uint_fast32_t j = jmtxd_matrix_brm_first_pos_in_row(out, i);
             j < jmtxd_matrix_brm_last_pos_in_row(out, i) + 1; ++j, ++k)
        {
                                      ...
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxd_matrix_cds_new(&out, r_out, c_out, 0, (int32_t[]){0}, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
