...
static uint32_t crs_get_row_entries(const jmtxd_matrix_crs *mtx, uint32_t row, uint32_t *pp_indices[1],
                                    double *pp_values[1])
{
    uint32_t offset, len;
    if (row == 0)
 ...
 */
static jmtx_result crs_insert_entry_at(jmtxd_matrix_crs *mtx, uint32_t row, uint32_t position, double value,
                                       uint32_t index)
{
...
jmtx_result jmtxd_matrix_crs_new(jmtxd_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
...
jmtx_result jmtxds_matrix_crs_new(jmtxd_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!p_mtx)
    {
...
jmtx_result jmtxd_matrix_crs_set_row(jmtxd_matrix_crs *mtx, uint32_t row, uint32_t n,
                                     const uint32_t indices[JMTX_ARRAY_ATTRIB(static n)],
                                     const double values[JMTX_ARRAY_ATTRIB(static n)])
{
...
jmtx_result jmtxds_matrix_crs_set_row(jmtxd_matrix_crs *mtx, uint32_t row, uint32_t n, const uint32_t *indices,
                                      const double *values)
{
    if (!mtx)
    {
...
uint32_t jmtxd_matrix_crs_get_row(const jmtxd_matrix_crs *mtx, uint32_t row, uint32_t *p_indices[1],
                                  double *p_elements[1])
{
    return crs_get_row_entries(mtx, row, p_indices, p_elements);
}

jmtx_result jmtxds_matrix_crs_get_row(const jmtxd_matrix_crs *mtx, uint32_t row, uint32_t *n, uint32_t **p_indices,
                                      double **p_elements)
{
    if (!mtx)
    {
...
jmtx_result jmtxd_matrix_crs_apply_unary_fn(const jmtxd_matrix_crs *mtx,
                                            int (*unary_fn)(uint32_t i, uint32_t j, double *p_value, void *param),
                                            void *param)
{
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
...
jmtx_result jmtxds_matrix_crs_apply_unary_fn(const jmtxd_matrix_crs *mtx,
                                             int (*unary_fn)(uint32_t i, uint32_t j, double *p_value, void *param),
                                             void *param)
{
    if (!mtx)
...
uint32_t jmtxd_matrix_crs_get_col(const jmtxd_matrix_crs *mtx, uint32_t col, uint32_t n,
                                  double p_values[JMTX_ARRAY_ATTRIB(n)], uint32_t p_rows[JMTX_ARRAY_ATTRIB(n)])
{
    uint32_t k = 0;
    for (uint32_t row = 0;
...
jmtx_result jmtxds_matrix_crs_get_col(const jmtxd_matrix_crs *mtx, uint32_t col, uint32_t n, uint32_t *p_count,
                                      double *p_values, uint32_t *p_rows)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxd_matrix_crs_transpose(const jmtxd_matrix_crs *mtx, jmtxd_matrix_crs **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
    ...
    jmtx_result res = jmtxd_matrix_crs_new(&out, mtx->base.cols, mtx->base.rows, mtx->n_entries, allocator_callbacks);
...
jmtx_result jmtxds_matrix_crs_transpose(const jmtxd_matrix_crs *mtx, jmtxd_matrix_crs **p_out,
                                        const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxd_matrix_crs_copy(const jmtxd_matrix_crs *mtx, jmtxd_matrix_crs **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
...
jmtx_result jmtxds_matrix_crs_copy(const jmtxd_matrix_crs *mtx, jmtxd_matrix_crs **p_out,
                                   const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxd_matrix_crs_build_row(jmtxd_matrix_crs *mtx, uint32_t row, uint32_t n,
                                       const uint32_t indices[JMTX_ARRAY_ATTRIB(static n)],
                                       const double values[JMTX_ARRAY_ATTRIB(static n)])
{
    jmtx_result res = JMTX_RESULT_SUCCESS;
...
jmtx_result jmtxds_matrix_crs_vector_multiply_row(const jmtxd_matrix_crs *mtx, const double *restrict x, uint32_t i,
                                                  double *restrict p_r)
{
    if (!mtx)
    {
...
jmtx_result jmtxd_matrix_crs_join_vertically(jmtxd_matrix_crs **output,
                                             const jmtx_allocator_callbacks *allocator_callbacks, unsigned k,
                                             const jmtxd_matrix_crs *matrix_list[JMTX_ARRAY_ATTRIB(static k)])
{
...
jmtx_result jmtxds_matrix_crs_join_vertically(jmtxd_matrix_crs **output,
