 ...
 */
void jmtxd_matrix_drm_givens_rotation_left(jmtxd_matrix_drm *mtx, unsigned r1, unsigned r2, const double ct,
                                           const double st)
{
    ...
    double *const row1 = mtx->values + n * r1;
    double *const row2 = mtx->values + n * r2;

    for (unsigned j = 0; j < n; ++j)
    {
        const double a = row1[j];
        const double b = row2[j];
        row1[j] = ct * a - st * b;
        row2[j] = st * a + ct * b;
    }
}
 ...
 */
jmtx_result jmtxds_matrix_drm_givens_rotation_left(jmtxd_matrix_drm *mtx, const unsigned r1, const unsigned r2,
                                                   double ct, double st)
{
    ...
    jmtxd_matrix_drm_givens_rotation_left(mtx, r1, r2, ct, st);
    return JMTX_RESULT_SUCCESS;
}
...
void jmtxd_matrix_drm_givens_rotation_right(jmtxd_matrix_drm *mtx, unsigned c1, unsigned c2, double ct, double st)
{
    ...
    double *const col1 = mtx->values + c1;
    double *const col2 = mtx->values + c2;

    if (!mtx->permutations)
    {
        for (unsigned i = 0; i < n; ++i)
        {
            const double a = col1[i * mtx->base.cols];
            const double b = col2[i * mtx->base.cols];
    ...
    else
    {
        for (unsigned i = 0; i < n; ++i)
        {
            const double a = col1[mtx->permutations[i] * mtx->base.cols];
            const double b = col2[mtx->permutations[i] * mtx->base.cols];
                                                    ...
                                                    double st)
{
    ...
    jmtxd_matrix_drm_givens_rotation_right(mtx, c1, c2, ct, st);
    return JMTX_RESULT_SUCCESS;
}
...
jmtx_result jmtxd_matrix_drm_multiply_matrix(jmtxd_matrix_drm *a, jmtxd_matrix_drm *b, jmtxd_matrix_drm *out)
{
    ...
    if (!b->permutations && !a->permutations)
    {
        ...
        {
            const double *restrict p_row = a->values + i * n;
            for (unsigned j = 0; j < new_cols; ++j)
            {
                double v = 0;
                const double *restrict p_col = b->values + j;
    ...
    else if (a->permutations && b->permutations)
    {
        for (unsigned i = 0; i < new_rows; ++i)
        {
            const double *restrict p_row = a->values + n * a->permutations[i];
            for (unsigned j = 0; j < new_cols; ++j)
            {
                double v = 0;
                const double *restrict p_col = b->values + j;
    ...
    else // if (b->permutations)
    {
        for (unsigned i = 0; i < new_rows; ++i)
        {
            const double *restrict p_row = a->values + n * i;
            for (unsigned j = 0; j < new_cols; ++j)
            {
                double v = 0;
                const double *restrict p_col = b->values + j;
...
jmtxd_matrix_drm jmtxd_matrix_drm_from_data(const unsigned rows, const unsigned cols,
                                            double values[JMTX_ARRAY_ATTRIB(static rows * cols)])
{
    return (jmtxd_matrix_drm){.base = {.type = JMTXD_TYPE_DRM,
                                       .rows = rows,
                                       .cols = cols,
