...
typedef struct JMTX_NAME_TYPED(matrix_ccs_struct) JMTX_NAME_TYPED(matrix_ccs);

struct JMTX_NAME_TYPED(matrix_ccs_struct)
{
    jmtx_matrix_base base;
    //  How many values exist in the columns left, so that column i is from index end_of_column_offsets[i] to
    //  end_of_column_offsets[i + 1]
    JMTX_INDEX_T *restrict end_of_column_offsets;
    //  Column indices corresponding with the individual values
    JMTX_INDEX_T *restrict indices;
    //  Values of values
...
/**
 * Version of JMTX_NAME_TYPED(matrix_ccs_set_col which does not touch the count of entries after the current column.
 * This is useful when building a new matrix, as it avoids unnecessary setting and resetting of these entries. Must be
 * called for each column in order to ensure that the matrix is properly built. Makes no checks on the input parameters
 * @param mtx pointer to the memory where the matrix is stored
 * @param col index of the column to set
 * @param n how many entries are in the column
...
/**
 * Removes entries exactly equal to zero. If the element is indeed zero, it is compared to (JMTX_SCALAR_T)0.0
 * @param mtx pointer to the memory where the matrix is stored
 */
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result JMTX_NAME_TYPED(matrix_ccs_copy)(const JMTX_NAME_TYPED(matrix_ccs) *restrict mtx,
                                             JMTX_NAME_TYPED(matrix_ccs) * *p_out,
                                             const jmtx_allocator_callbacks *allocator_callbacks);
/**
