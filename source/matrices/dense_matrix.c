...
static scalar_t dense_row_view_element_at(MatrixView this, uint32_t i, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const jmtx_dense_matrix_struct* const parent = (jmtx_dense_matrix_struct*)view->parent;
    const scalar_t* const ptr = parent->data + view->offset;
    return ptr[j + i * parent->columns];
}

static uint32_t dense_row_view_elements_in_row(MatrixView this, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    return view->columns;
}

static uint32_t dense_row_view_elements_in_column(MatrixView this, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    return view->rows;
}

static void dense_row_view_get_row_elements(MatrixView this, scalar_t* const elements, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    memcpy(elements, view->offset + ((jmtx_dense_matrix_struct*)view->parent)->data + view->parent->columns * i, view->columns * sizeof(scalar_t));
}

static void dense_row_view_get_col_elements(MatrixView this, scalar_t* const elements, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const scalar_t* const ptr = ((jmtx_dense_matrix_struct*)view->parent)->data + view->offset;
    for (uint32_t i = 0; i < view->rows; ++i)
    {
...
{
    DenseMatrixView view = (DenseMatrixView)this;
    return ((jmtx_dense_matrix_struct*)view->parent)->data + view->offset;
}

static void dense_row_view_get_row_indices(MatrixView this, uint32_t* const indices, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    for (uint32_t j = 0; j < view->columns; ++j)
    {
        indices[j] = j + view->offset + i * view->parent->columns;
    }
}

static void dense_row_view_get_col_indices(MatrixView this, uint32_t* const indices, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    for (uint32_t i = 0; i < view->rows; ++i)
    {
...
static int dense_row_view_element_at_if(MatrixView this, uint32_t i, uint32_t j, scalar_t* ptr)
{
    DenseMatrixView view = (DenseMatrixView)this;
    if (ptr)
    {
...
static scalar_t dense_row_view_element_as_flat(MatrixView this, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const uint32_t i = j / view->columns;
    j %= view->columns;
...
{
    DenseMatrixView view = (DenseMatrixView)this;
    jmtx_dense_matrix_struct* const mtx = malloc(sizeof*mtx);
    if (!mtx) return NULL;
...
static scalar_t dense_col_view_element_at(MatrixView this, uint32_t i, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const jmtx_dense_matrix_struct* const parent = (jmtx_dense_matrix_struct*)view->parent;
    const scalar_t* const ptr = parent->data + view->offset;
...
static uint32_t dense_col_view_elements_in_row(MatrixView this, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    return view->columns;
}

static uint32_t dense_col_view_elements_in_column(MatrixView this, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    return view->rows;
}

static void dense_col_view_get_col_elements(MatrixView this, scalar_t* const elements, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    memcpy(elements, view->offset + ((jmtx_dense_matrix_struct*)view->parent)->data + view->parent->rows * i, view->rows * sizeof(scalar_t));
}

static void dense_col_view_get_row_elements(MatrixView this, scalar_t* const elements, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const scalar_t* const ptr = ((jmtx_dense_matrix_struct*)view->parent)->data + view->offset;
    for (uint32_t i = 0; i < view->rows; ++i)
...
static void dense_col_view_get_row_indices(MatrixView this, uint32_t* const indices, uint32_t i)
{
    DenseMatrixView view = (DenseMatrixView)this;
    for (uint32_t j = 0; j < view->columns; ++j)
    {
...
static void dense_col_view_get_col_indices(MatrixView this, uint32_t* const indices, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    for (uint32_t i = 0; i < view->rows; ++i)
    {
...
static int dense_col_view_element_at_if(MatrixView this, uint32_t i, uint32_t j, scalar_t* ptr)
{
    DenseMatrixView view = (DenseMatrixView)this;
    if (ptr)
    {
...
static scalar_t dense_col_view_element_as_flat(MatrixView this, uint32_t j)
{
    DenseMatrixView view = (DenseMatrixView)this;
    const uint32_t i = j / view->rows;
    j %= view->rows;
...
static Matrix dense_col_view_copy(MatrixView this)
{
    ...
    for (uint32_t i = 0; i < view->columns; ++i)
    {
...
static scalar_t dense_row_element_at(Matrix mtx, uint32_t i, uint32_t j)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
    return this->data[j + i * this->rows];
}

static void dense_row_get_row_elements(Matrix mtx, scalar_t* const elements, uint32_t i)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
    memcpy(elements, this->data + i * this->columns, this->columns * sizeof(*this->data));
}

static void dense_row_get_col_elements(Matrix mtx, scalar_t* const elements, uint32_t j)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
...
static void dense_row_get_row_indices(Matrix mtx, uint32_t* const indices, uint32_t i)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
...
static void dense_row_get_col_indices(Matrix mtx, uint32_t* const indices, uint32_t j)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
    for (uint32_t i = 0; i < this->rows; ++i)
    {
...
static int dense_row_element_at_if(Matrix mtx, uint32_t i, uint32_t j, scalar_t* ptr)
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
    if (ptr)
    {
...
{
    const jmtx_dense_matrix_struct* const this = (jmtx_dense_matrix_struct*)mtx;
    jmtx_dense_matrix_struct* const new_mtx = malloc(sizeof*new_mtx);
...
//        for (uint32_t i = 0; i < res->elements; ++i)
//        {
//            res->data[i] += b->data[i];
//        }
//    }
...
//        {
//            for (uint32_t j = 0; j < res->columns; ++j)
//            {
...
//    uint32_t* const indices = calloc(2 * mtx->columns, sizeof*indices);
//    if (!indices)
//    {
//        free(this->data);
//        free(this);
//        return NULL;
//    }
//    uint32_t* const indices2 = indices + mtx->columns;
...
//            scalar_t v = 0.0f;
//            for (uint32_t k = 0; k < mtx->columns; ++k)
//            {
...
static MatrixView dense_row_slice(Matrix this, uint32_t begin_row, uint32_t end_row, uint32_t begin_col, uint32_t end_col)
{
    ...
    view->offset = begin_col + begin_row * ((jmtx_dense_matrix_struct*)this)->columns;
    view->functions = &dense_row_view_vtbl;
    return (MatrixView)view;
}

static MatrixView dense_col_slice(Matrix this, uint32_t begin_row, uint32_t end_row, uint32_t begin_col, uint32_t end_col)
{
    DenseMatrixView view = malloc(sizeof *view);
    if (!view) return NULL;
    view->parent = this;
    view->columns = end_col - begin_col;
    view->rows = end_row - begin_row;
    view->offset = begin_row + begin_col * ((jmtx_dense_matrix_struct*)this)->rows;
