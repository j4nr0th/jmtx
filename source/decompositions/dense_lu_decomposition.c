#include "dense_lu_decomposition.h"
...
void JMTX_NAME_TYPED(decompose_lu_drm)(JMTX_NAME_TYPED(matrix_drm) * mtx, JMTX_NAME_TYPED(matrix_drm) * decomposed)
{
    ...
    for (JMTX_INDEX_T i = 0; i < n; ++i)
    {
        ...
        for (JMTX_FAST_INT_T j = 0; j < i; ++j)
        {
            JMTX_SCALAR_T v = 0;
            //  Row of L
            const JMTX_SCALAR_T *li = decomposed->values + n * i;
            //  Column of U
            const JMTX_SCALAR_T *uj = decomposed->values + j;
            for (JMTX_FAST_INT_T k = 0; k < j; ++k)
            {
                v += li[k] * uj[k * n];
            }
        ...
        for (JMTX_FAST_INT_T j = 0; j <= i; ++j)
        {
            JMTX_SCALAR_T v = 0;
            //  Row of L
            const JMTX_SCALAR_T *lj = decomposed->values + n * j;
            //  Column of U
            const JMTX_SCALAR_T *ui = decomposed->values + i;
            for (JMTX_FAST_INT_T k = 0; k < j; ++k)
            {
                v += lj[k] * ui[k * n];
            }
                                                    ...
                                                    JMTX_NAME_TYPED(matrix_drm) * decomposed)
{
    ...
    for (JMTX_INDEX_T i = 0; i < n; ++i)
    {
        ...
        JMTX_INDEX_T pivot = i;
        for (JMTX_INDEX_T j = i; j < n; ++j)
        {
            if (JMTX_ABS(decomposed->values[n * decomposed->permutations[j] + i]) >
                JMTX_ABS(decomposed->values[n * decomposed->permutations[pivot] + i]))
            {
                pivot = j;
            }
        }
        if (pivot != i && pivot != n)
        {
            (void)JMTX_NAME_TYPED(matrix_drm_swap_rows)(decomposed, i, pivot);
        }
        ...
        for (JMTX_FAST_INT_T j = 0; j < i; ++j)
        {
            JMTX_SCALAR_T v = 0;
            //  Row of L
            JMTX_SCALAR_T *li = decomposed->values + n * decomposed->permutations[i];
            //  Column of U
            const JMTX_SCALAR_T *uj = decomposed->values + j;
            for (JMTX_FAST_INT_T k = 0; k < j; ++k)
            {
        ...
        for (JMTX_FAST_INT_T j = 0; j <= i; ++j)
        {
            JMTX_SCALAR_T v = 0;
            //  Row of L
            const JMTX_SCALAR_T *lj = decomposed->values + n * decomposed->permutations[j];
            //  Column of U
            JMTX_SCALAR_T *ui = decomposed->values + i;
            for (JMTX_FAST_INT_T k = 0; k < j; ++k)
            {
