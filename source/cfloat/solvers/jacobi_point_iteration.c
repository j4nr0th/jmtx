...
jmtx_result jmtxcs_solve_iterative_jacobi_crs(
        const jmtxc_matrix_crs* mtx, const uint32_t n, const _Complex float y[JMTX_ARRAY_ATTRIB(static restrict n)], _Complex float x[JMTX_ARRAY_ATTRIB(restrict n)], _Complex float aux_vec1[JMTX_ARRAY_ATTRIB(restrict n)], _Complex float aux_vec2[JMTX_ARRAY_ATTRIB(restrict n)],
        jmtx_solver_arguments* args)
{
    if (!mtx)
    {
