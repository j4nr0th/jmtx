// Automatically generated from source/float/solvers/jacobi_point_iteration.c on Fri Dec  1 17:36:03 2023
//
...
jmtx_result jmtxc_jacobi_relaxed_crs(
        const jmtxc_matrix_crs* mtx, const _Complex float* restrict y, _Complex float* restrict x, float relaxation_factor, _Complex float* restrict aux_vec1,
...
#pragma omp parallel for shared(mtx, zero_diag, n, x, y, aux_vector1) reduction(+:y_mag) default(none)
    ...
    {
        _Complex float d;
        if ((d = jmtxc_matrix_crs_get_entry(mtx, i, i)) == CMPLXF(0.0f, 0.0f))
        {
            //  Diagonal entry is zero!
            //  Can't solve this one with Jacobi
            zero_diag += 1;
