// Automatically generated from source/float/solvers/incomplete_lu_decomposition.c on Fri Dec  1 17:36:03 2023
//
...
jmtx_result jmtxc_incomplete_lu_crs(
...
jmtx_result jmtxc_incomplete_lu_crs_parallel(
        jmtxc_matrix_crs* a, jmtxc_matrix_crs** p_l, jmtxc_matrix_ccs** p_u,
        const jmtx_allocator_callbacks* allocator_callbacks, jmtx_solver_arguments* args)
{
    ...
    _Complex float* column_values = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*column_values) * max_elements_in_direction);
    if (!column_values)
    {
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        ...
        pos += 1;
        //  Values bellow the diagonal go to L
        res = jmtxc_matrix_crs_build_row(l, i, pos, column_indices, column_values);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, column_values);
            allocator_callbacks->free(allocator_callbacks->state, column_indices);
        ...
        //  Get column values from the matrix A
        c = jmtxc_matrix_crs_get_col(a, i, max_elements_in_direction, column_values, column_indices);
        ...
        //  Values above the diagonal go to U
        res = jmtxc_matrix_ccs_build_col(u, i, pos + 1, indices, values);
        if (res != JMTX_RESULT_SUCCESS)
        {
            allocator_callbacks->free(allocator_callbacks->state, column_values);
            allocator_callbacks->free(allocator_callbacks->state, column_indices);
    ...
    const float convergence = args->in_convergence_criterion;
    float* const p_err = args->opt_error_evolution;
...
max_iterations, allocator_callbacks, a, l, u, n, convergence, converged, any_failed_allocating, p_err)
    {
        ...
        _Complex float* const col_val = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*col_val) * max_elements_in_direction);
        if (!col_idx || !col_val)
        {
...
#pragma omp for schedule(static)
                for (uint32_t p = 0; p < n; ++p)
                {
                    ...
                    uint32_t* positions;
                    _Complex float* values;
                    n_items = jmtxc_matrix_crs_get_row(a, p, &positions, &values);
                    uint32_t out_items;
                    uint32_t* out_positions;
                    _Complex float* out_values;
                    out_items = jmtxc_matrix_crs_get_row(l, p, &out_positions, &out_values);
                    ...
                    for (uint32_t r = 0, k = 0; r < n_items && (int) positions[r] < (int) p; ++r, ++k)
                    {
                        ...
                        v = (va - v) / u_val[u_col_count - 1];
                        const float relative_change = cabsf((v - out_values[k]) / out_values[k]);
                    ...
                    n_items =  jmtxc_matrix_crs_get_col(a, p, max_elements_in_direction, col_val, col_idx);
                    out_items = jmtxc_matrix_ccs_get_col(u, p, &out_positions, &out_values);
                    //  Compute the product of row m of matrix L and column p of matrix U to update the entry U_mp
                    for (uint32_t r = 0, k = 0; r < n_items && col_idx[r] < p + 1; ++r, ++k)
                    {
                        const uint32_t m = col_idx[r];
                        _Complex float v = 0;
                        _Complex float va = col_val[r];
                        uint32_t l_row_count;
                        uint32_t* l_col_indices;
                        _Complex float* l_val;
                        l_row_count = jmtxc_matrix_crs_get_row(l, m, &l_col_indices, &l_val);
...
jmtx_result jmtxc_incomplete_lu_crs_old(
        const jmtxc_matrix_crs* a, jmtxc_matrix_crs** p_l, jmtxc_matrix_ccs** p_u, float convergence, uint32_t max_iterations,
        _Complex float* final_max_change, uint32_t* p_last_iteration, const jmtx_allocator_callbacks* allocator_callbacks)
{
    if (!a)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    while (iteration_count < max_iterations)
    {
        ...
        for (uint32_t p = 0; p < n; ++p)
        {
            ...
            uint32_t* positions;
            _Complex float* values;
            uint32_t n_items = jmtxc_matrix_crs_get_row(a, p, &positions, &values);
            uint32_t* out_positions;
            _Complex float* out_values;
            uint32_t out_items = jmtxc_matrix_crs_get_row(l, p, &out_positions, &out_values);
            //  Loop over elements of L which can be updated
            for (uint32_t r = 0, k = 0; r < n_items && (int)positions[r] < (int)p; ++r, ++k)
            {
                const uint32_t m = positions[r];
...
#ifndef NDEBUG
                _Complex float v1 = jmtxc_matrix_crs_get_entry(l, p, m);
                assert(v1 != 0.0f);
#endif
//                jmtxc_matrix_crs_set_entry(l, p, m, v);
                out_values[k] = v;
            }

            //  Update the p-th column of U
            n_items = jmtxc_matrix_crs_get_col(a, p, max_elements_in_direction, column_values, column_indices);
            out_items = jmtxc_matrix_ccs_get_col(u, p, &out_positions, &out_values);
            //  Compute the product of row m of matrix L and column p of matrix U to update the entry U_mp
            for (uint32_t r = 0, k = 0; r < n_items && column_indices[r] < p + 1; ++r, ++k)
            {
                ...
                assert(out_positions[k] == m);
                _Complex float v = 0;
                _Complex float va = column_values[r];
                assert(va != 0.0f);
                uint32_t* l_col_indices;
                _Complex float* l_val;
                assert(va != 0.0f);
...
#ifndef NDEBUG
                _Complex float v1 = jmtxc_matrix_ccs_get_entry(u, m, p);
                assert(v1 != 0.0f);
#endif
//                jmtxc_matrix_ccs_set_entry(u, m, p, v);
                out_values[k] = v;
