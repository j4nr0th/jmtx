...
static uint32_t ccs_get_column_entries(const jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t **pp_indices,
                                       _Complex float **pp_values)
{
    uint32_t offset, len;
    if (col == 0)
    {
 ...
 */
static jmtx_result ccs_insert_entry_at(jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t position, _Complex float value,
                                       uint32_t index)
{
...
jmtx_result jmtxc_matrix_ccs_new(jmtxc_matrix_ccs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
...
jmtx_result jmtxcs_matrix_ccs_new(jmtxc_matrix_ccs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!p_mtx)
    {
...
jmtx_result jmtxc_matrix_ccs_set_col(jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t n, const uint32_t *indices,
...
jmtx_result jmtxcs_matrix_ccs_set_col(jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t n, const uint32_t *indices,
                                      const _Complex float *values)
{
    if (!mtx)
    {
...
void jmtxc_matrix_ccs_vector_multiply(const jmtxc_matrix_ccs *mtx, const _Complex float *restrict x,
...
jmtx_result jmtxcs_matrix_ccs_vector_multiply(const jmtxc_matrix_ccs *mtx, const _Complex float *restrict x,
                                              _Complex float *restrict y)
{
    if (!mtx)
    {
...
uint32_t jmtxc_matrix_ccs_get_col(const jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t **p_indices,
                                  _Complex float **p_elements)
{
    return ccs_get_column_entries(mtx, col, p_indices, p_elements);
}

jmtx_result jmtxcs_matrix_ccs_get_col(const jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t *n, uint32_t **p_indices,
                                      _Complex float **p_elements)
{
    if (!mtx)
    {
...
jmtx_result jmtxc_matrix_ccs_apply_unary_fn(const jmtxc_matrix_ccs *mtx,
                                            int (*unary_fn)(uint32_t i, uint32_t j, _Complex float *p_element,
                                                            void *param),
                                            void *param)
{
...
jmtx_result jmtxcs_matrix_ccs_apply_unary_fn(const jmtxc_matrix_ccs *mtx,
                                             int (*unary_fn)(uint32_t i, uint32_t j, _Complex float *p_element,
                                                             void *param),
                                             void *param)
{
...
uint32_t jmtxc_matrix_ccs_get_row(const jmtxc_matrix_ccs *mtx, uint32_t row, uint32_t n, _Complex float *p_values,
                                  uint32_t *p_columns)
{
    uint32_t k = 0;
    for (uint32_t col = 0;
...
jmtx_result jmtxcs_matrix_ccs_get_row(const jmtxc_matrix_ccs *mtx, uint32_t row, uint32_t n, _Complex float *p_values,
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxc_matrix_ccs_new(&out, mtx->base.cols, mtx->base.rows, mtx->n_entries, allocator_callbacks);
...
jmtx_result jmtxc_matrix_ccs_copy(const jmtxc_matrix_ccs *mtx, jmtxc_matrix_ccs **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
...
jmtx_result jmtxcs_matrix_ccs_copy(const jmtxc_matrix_ccs *mtx, jmtxc_matrix_ccs **p_out,
                                   const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxcs_matrix_zero_all_entries(const jmtxc_matrix_ccs *mtx)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxc_matrix_zero_all_entries(mtx);
...
jmtx_result jmtxcs_matrix_ccs_set_all_entries(jmtxc_matrix_ccs *mtx, _Complex float x)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
    ...
    jmtxc_matrix_ccs_set_all_entries(mtx, x);
    return JMTX_RESULT_SUCCESS;
}

jmtx_result jmtxc_matrix_ccs_build_col(jmtxc_matrix_ccs *mtx, uint32_t col, uint32_t n, const uint32_t *indices,
                                       const _Complex float *values)
{
