        ...
        const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    return jmtxc_matrix_crs_new(p_mtx, cols, rows, reserved_entries, allocator_callbacks);
}
        ...
        const jmtxc_matrix_crs* mtx, jmtxc_matrix_crs** p_out, const jmtx_allocator_callbacks* allocator_callbacks)
{
    ...
    *column_cum_counts = 1;
    ...
    {
        n = jmtxc_matrix_crs_entries_in_col(mtx, j);
        (void) jmtxc_matrix_crs_get_col(mtx, j, n, new_values + p, new_indices + p);
...
jmtx_result jmtxc_matrix_crs_join_vertically(jmtxc_matrix_crs** output, const jmtx_allocator_callbacks* allocators,
                                            unsigned k, const jmtxc_matrix_crs* matrix_list[static k])
{
    ...
    jmtx_result res = jmtxc_matrix_crs_new(&out, col_count, n_rows, element_count, allocators);
...
jmtx_result jmtxcs_matrix_crs_join_vertically(
        jmtxc_matrix_crs** output, const jmtx_allocator_callbacks* allocators, unsigned int k,
        const jmtxc_matrix_crs** matrix_list)
{
    ...
    return jmtxc_matrix_crs_join_vertically(output, allocators, k, matrix_list);
}
