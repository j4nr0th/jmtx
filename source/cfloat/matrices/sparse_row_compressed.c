...
static uint32_t crs_get_row_entries(const jmtxc_matrix_crs *mtx, uint32_t row, uint32_t *pp_indices[1],
                                    _Complex float *pp_values[1])
{
    uint32_t offset, len;
    if (row == 0)
    {
 ...
 */
static jmtx_result crs_insert_entry_at(jmtxc_matrix_crs *mtx, uint32_t row, uint32_t position, _Complex float value,
                                       uint32_t index)
{
    ...
    if (mtx->capacity == mtx->n_entries)
    {
        ...
        _Complex float *const new_values = mtx->base.allocator_callbacks.realloc(
...
jmtx_result jmtxc_matrix_crs_new(jmtxc_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
...
jmtx_result jmtxcs_matrix_crs_new(jmtxc_matrix_crs **p_mtx, uint32_t rows, uint32_t cols, uint32_t reserved_entries,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!p_mtx)
    {
...
jmtx_result jmtxc_matrix_crs_shrink(jmtxc_matrix_crs *mtx)
{
    ...
    _Complex float *element_new_ptr = mtx->base.allocator_callbacks.realloc(
...
jmtx_result jmtxc_matrix_crs_set_row(jmtxc_matrix_crs *mtx, uint32_t row, uint32_t n,
                                     ...
                                     const _Complex float values[JMTX_ARRAY_ATTRIB(static n)])
{
    ...
    {
        _Complex float *new_element_ptr = mtx->base.allocator_callbacks.realloc(
...
jmtx_result jmtxcs_matrix_crs_set_row(jmtxc_matrix_crs *mtx, uint32_t row, uint32_t n, const uint32_t *indices,
                                      const _Complex float *values)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
void jmtxc_matrix_crs_vector_multiply(const jmtxc_matrix_crs *mtx, const _Complex float *restrict x,
...
jmtx_result jmtxcs_matrix_crs_vector_multiply(const jmtxc_matrix_crs *mtx, const _Complex float *restrict x,
                                              _Complex float *restrict y)
{
    if (!mtx)
    {
...
uint32_t jmtxc_matrix_crs_get_row(const jmtxc_matrix_crs *mtx, uint32_t row, uint32_t *p_indices[1],
                                  _Complex float *p_elements[1])
{
    return crs_get_row_entries(mtx, row, p_indices, p_elements);
}

jmtx_result jmtxcs_matrix_crs_get_row(const jmtxc_matrix_crs *mtx, uint32_t row, uint32_t *n, uint32_t **p_indices,
...
jmtx_result jmtxc_matrix_crs_apply_unary_fn(const jmtxc_matrix_crs *mtx,
                                            int (*unary_fn)(uint32_t i, uint32_t j, _Complex float *p_value,
                                                            void *param),
                                            void *param)
{
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
...
jmtx_result jmtxcs_matrix_crs_apply_unary_fn(const jmtxc_matrix_crs *mtx,
                                             int (*unary_fn)(uint32_t i, uint32_t j, _Complex float *p_value,
                                                             void *param),
                                             void *param)
{
...
uint32_t jmtxc_matrix_crs_get_col(const jmtxc_matrix_crs *mtx, uint32_t col, uint32_t n,
                                  _Complex float p_values[JMTX_ARRAY_ATTRIB(n)], uint32_t p_rows[JMTX_ARRAY_ATTRIB(n)])
{
    uint32_t k = 0;
    for (uint32_t row = 0;
...
jmtx_result jmtxcs_matrix_crs_get_col(const jmtxc_matrix_crs *mtx, uint32_t col, uint32_t n, uint32_t *p_count,
                                      _Complex float *p_values, uint32_t *p_rows)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxc_matrix_crs_transpose(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
    ...
    jmtx_result res = jmtxc_matrix_crs_new(&out, mtx->base.cols, mtx->base.rows, mtx->n_entries, allocator_callbacks);
...
jmtx_result jmtxcs_matrix_crs_transpose(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                        const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxc_matrix_crs_copy(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                  const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
...
jmtx_result jmtxcs_matrix_crs_copy(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                   const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (!mtx)
    {
...
jmtx_result jmtxc_matrix_crs_build_row(jmtxc_matrix_crs *mtx, uint32_t row, uint32_t n,
                                       const uint32_t indices[JMTX_ARRAY_ATTRIB(static n)],
                                       const _Complex float values[JMTX_ARRAY_ATTRIB(static n)])
{
    jmtx_result res = JMTX_RESULT_SUCCESS;
...
jmtx_result jmtxcs_matrix_crs_vector_multiply_row(const jmtxc_matrix_crs *mtx, const _Complex float *restrict x,
                                                  uint32_t i, _Complex float *restrict p_r)
{
    if (!mtx)
    {
        return JMTX_RESULT_NULL_PARAM;
...
jmtx_result jmtxc_matrix_crs_join_vertically(jmtxc_matrix_crs **output,
                                             const jmtx_allocator_callbacks *allocator_callbacks, unsigned k,
                                             const jmtxc_matrix_crs *matrix_list[JMTX_ARRAY_ATTRIB(static k)])
{
...
jmtx_result jmtxc_matrix_crs_new_like(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks, const _Complex float *p_val)
{
    if (allocator_callbacks == NULL)
    {
        allocator_callbacks = &JMTX_DEFAULT_ALLOCATOR_CALLBACKS;
...
jmtx_result jmtxcs_matrix_crs_new_like(const jmtxc_matrix_crs *mtx, jmtxc_matrix_crs **p_out,
                                       const jmtx_allocator_callbacks *allocator_callbacks, const _Complex float *p_val)
{
    if (!mtx)
    {
...
jmtx_result jmtxcs_matrix_crs_join_vertically(jmtxc_matrix_crs **output,
