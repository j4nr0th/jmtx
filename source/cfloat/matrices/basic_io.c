// Automatically generated from source/float/matrices/basic_io.c on Fri Dec  1 17:36:03 2023
//
...
jmtx_result jmtxc_matrix_crs_to_file_explicit(const jmtxc_matrix_crs* mtx, const char* filename)
{
    ...
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        ...
        for (uint32_t j = 0, k = 0; j < mtx->base.cols; ++j)
        {
            ...
            {
                fprintf(f_out, "%+.8e%+.8ej\t", crealf(elements[j]), cimagf(elements[j]));
            }
            else
            {
...

{
    ...
    for (uint32_t i = 0; i < mtx->base.rows; ++i)
    {
        ...
        {
            const _Complex float v = jmtxc_matrix_ccs_get_entry(mtx, i, j);
            fprintf(f_out, "%+.8e%+.8ej\t", crealf(v), cimagf(v));
        }
        fprintf(f_out, "\n");
    }
