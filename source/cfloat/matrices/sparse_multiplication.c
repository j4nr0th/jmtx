// Automatically generated from source/float/matrices/sparse_multiplication.c on Fri Dec  1 17:36:03 2023
//
 ...
 */
jmtx_result jmtxc_multiply_matrix_crs(const jmtxc_matrix_crs *a, const jmtxc_matrix_ccs *b, jmtxc_matrix_crs **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxc_matrix_crs_new(&out, r_out, c_out, a->n_entries + b->n_entries, allocator_callbacks);
    ...
    _Complex float *values = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*values) * capacity);
    if (!values)
    {
    ...
    for (uint32_t i = 0; i < r_out; ++i)
    {
        ...
        uint32_t n_a, n_b;
        uint32_t *i_a, *i_b;
        _Complex float *v_a, *v_b;
        n_a = jmtxc_matrix_crs_get_row(a, i, &i_a, &v_a);
        for (uint32_t j = 0; j < c_out; ++j)
        {
            n_b = jmtxc_matrix_ccs_get_col(b, j, &i_b, &v_b);
            _Complex float v = 0;
            ...
            if (v != 0)
            {
                if (count == capacity)
                {
                    ...
                    _Complex float *const new_v = allocator_callbacks->realloc(allocator_callbacks->state, values,
        ...
        jmtxc_matrix_crs_build_row(out, i, count, indices, values);
    }
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxc_multiply_matrix_crs(a, b, p_out, allocator_callbacks);
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on allocation failure
 */
jmtx_result jmtxc_multiply_matrix_ccs(const jmtxc_matrix_crs *a, const jmtxc_matrix_ccs *b, jmtxc_matrix_ccs **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    if (allocator_callbacks == NULL)
    {
    ...
    jmtxc_matrix_ccs *out;
    jmtx_result res = jmtxc_matrix_ccs_new(&out, r_out, c_out, a->n_entries + b->n_entries, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
    ...
    for (uint32_t j = 0; j < c_out; ++j)
    {
        ...
        jmtxc_matrix_ccs_build_col(out, j, count, indices, values);
    }
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxc_multiply_matrix_ccs(a, b, p_out, allocator_callbacks);
}
 ...
 */
_Complex float jmtxc_multiply_matrix_sparse_vectors(uint32_t n_a,
                                                    const uint32_t i_a[JMTX_ARRAY_ATTRIB(const static n_a)],
                                                    const _Complex float v_a[JMTX_ARRAY_ATTRIB(const static n_a)],
                                                    uint32_t n_b,
                                                    const uint32_t i_b[JMTX_ARRAY_ATTRIB(const static n_b)],
                                                    const _Complex float v_b[JMTX_ARRAY_ATTRIB(const static n_b)])
{
    _Complex float v = 0;
    uint32_t ia = 0, ib = 0;
    while (ia < n_a && ib < n_b)
...
_Complex float jmtxc_multiply_matrix_sparse_vectors_limit(uint32_t max_a, uint32_t max_b, uint32_t n_a,
                                                          ...
                                                          const _Complex float v_a[JMTX_ARRAY_ATTRIB(static max_a)],
                                                          uint32_t n_b,
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on allocation failure
 */
jmtx_result jmtxc_multiply_matrix_brm(const jmtxc_matrix_brm *a, const jmtxc_matrix_brm *b, jmtxc_matrix_brm **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtxc_matrix_brm *out;
    jmtx_result res = jmtxc_matrix_brm_new(&out, r_out, c_out, max_ubw, max_lbw, NULL, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
        return res;
    }
    ...
    _Complex float *col_vals = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*col_vals) * capacity);
    if (!col_vals)
    {
        jmtxc_matrix_brm_destroy(out);
        return JMTX_RESULT_BAD_ALLOC;
    }
    _Complex float *values = allocator_callbacks->alloc(allocator_callbacks->state, sizeof(*col_vals) * capacity);
    if (!values)
    {
    ...
    for (uint32_t i = 0; i < r_out; ++i)
    {
        ...
        uint32_t n_a, n_b;
        //        uint32_t* i_a, *i_b;
        _Complex float *v_a; //, *v_b = col_vals;
        n_a = jmtxc_matrix_brm_get_row(a, i, &v_a);
        (void)n_a, (void)n_b;
        const uint_fast32_t first_a = jmtxc_matrix_brm_first_pos_in_row(a, i);
        const uint_fast32_t last_a = jmtxc_matrix_brm_last_pos_in_row(a, i);
        for (uint_fast32_t j = jmtxc_matrix_brm_first_pos_in_row(out, i);
             j < jmtxc_matrix_brm_last_pos_in_row(out, i) + 1; ++j, ++k)
        {
            n_b = jmtxc_matrix_brm_get_col(b, j, col_vals);
            const uint_fast32_t first_b = jmtxc_matrix_brm_first_pos_in_col(b, j);
            const uint_fast32_t last_b = jmtxc_matrix_brm_last_pos_in_col(b, j);
        ...
        jmtxc_matrix_brm_set_row(out, i, values);
    }
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxc_multiply_matrix_brm(a, b, p_out, allocator_callbacks);
}
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on allocation failure
 */
jmtx_result jmtxc_multiply_matrix_cds(const jmtxc_matrix_cds *a, const jmtxc_matrix_cds *b, jmtxc_matrix_cds **p_out,
                                      const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    jmtx_result res = jmtxc_matrix_cds_new(&out, r_out, c_out, 0, (int32_t[]){0}, allocator_callbacks);
    if (res != JMTX_RESULT_SUCCESS)
    {
    ...
    for (uint32_t i = 0; i < r_out; ++i)
    {
        ...
        for (uint_fast32_t j = 0; j < c_out; ++j)
        {
            ...
            const _Complex float val = jmtxc_multiply_matrix_sparse_vectors(c_a, idx_a, val_a, c_b, idx_b, val_b);
            if (val != 0)
            {
                                       ...
                                       const jmtx_allocator_callbacks *allocator_callbacks)
{
    ...
    return jmtxc_multiply_matrix_cds(a, b, p_out, allocator_callbacks);
}
