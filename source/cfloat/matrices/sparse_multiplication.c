 ...
 */
jmtx_result jmtxc_multiply_matrix_crs(
        ...
        for (uint32_t j = 0; j < c_out; ++j)
        {
            ...
            if (v != 0)
            {
                if (count == capacity)
                {
                    ...
                    _Complex float* const new_v = allocator_callbacks->realloc(allocator_callbacks->state, values, sizeof(*values) * new_capacity);
                    if (!new_v)
                    {
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on allocation failure
 */
jmtx_result jmtxc_multiply_matrix_ccs(
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on allocation failure
 */
jmtx_result jmtxc_multiply_matrix_brm(
...
//        uint32_t* i_a, *i_b;
        ...
        for (uint_fast32_t j = jmtxc_matrix_brm_first_pos_in_row(out, i);
             j < jmtxc_matrix_brm_last_pos_in_row(out, i) + 1;
             ++j, ++k)
        {
