 ...
 */
JMTX_NODISCARD_FUNCTION
jmtx_result jmtxcs_matrix_crs_set_row(jmtxc_matrix_crs* mtx, uint32_t row, uint32_t n, const uint32_t indices[static n], const _Complex float values[static n]);
...
jmtx_result jmtxcs_matrix_crs_get_col(
        const jmtxc_matrix_crs* mtx, uint32_t col, uint32_t n, uint32_t* p_count, _Complex float p_values[n], uint32_t p_rows[n]);
 ...
 */
JMTX_NODISCARD_FUNCTION
jmtx_result jmtxcs_matrix_crs_join_vertically(jmtxc_matrix_crs** output, const jmtx_allocator_callbacks* allocators, unsigned k, const jmtxc_matrix_crs* matrix_list[static k]);
