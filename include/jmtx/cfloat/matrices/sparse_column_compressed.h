...
/**
 * Version of jmtxc_matrix_ccs_set_col which does not touch the count of entries after the current column. This is useful
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxc_matrix_ccs_build_col(jmtxc_matrix_ccs* mtx, uint32_t col, uint32_t n, const uint32_t* indices, const _Complex float* values);
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxc_matrix_ccs_copy(const jmtxc_matrix_ccs* restrict mtx, jmtxc_matrix_ccs** p_out, const jmtx_allocator_callbacks* allocator_callbacks);
