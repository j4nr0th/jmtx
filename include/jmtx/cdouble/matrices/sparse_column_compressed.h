...
/**
 * Version of jmtxz_matrix_ccs_set_col which does not touch the count of entries after the current column. This is useful
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_ccs_build_col(jmtxz_matrix_ccs* mtx, uint32_t col, uint32_t n, const uint32_t* indices, const _Complex double* values);
...
/**
 * Similar to jmtxz_matrix_crs_zero_all_entries, but slower, since it can not use memset. On the other hand, it allows for
 ...
 * @return JMTX_RESULT_SUCCESS if successful, JMTX_RESULT_BAD_ALLOC on memory allocation failure
 */
jmtx_result jmtxz_matrix_ccs_copy(const jmtxz_matrix_ccs* restrict mtx, jmtxz_matrix_ccs** p_out, const jmtx_allocator_callbacks* allocator_callbacks);
