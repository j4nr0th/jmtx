...
int main()
{
    ...
    jmtx_scalar_t X[TEST_MATRIX_MAJOR_DIM];
    jmtx_scalar_t Y_crs[TEST_MATRIX_MAJOR_DIM], Y_ccs[TEST_MATRIX_MAJOR_DIM];
    jmtx_matrix_crs crs_matrix_1;
    jmtx_matrix_crs crs_matrix_2;
    matrix_crs_new(&crs_matrix_1, TEST_MATRIX_MINOR_DIM, TEST_MATRIX_MAJOR_DIM, 0, NULL);
    jmtx_matrix_ccs ccs_matrix_1;
//    jmtx_matrix_ccs ccs_matrix_2;
    matrix_ccs_new(&ccs_matrix_1, TEST_MATRIX_MAJOR_DIM, TEST_MATRIX_MINOR_DIM, 0, NULL);

    for (uint32_t i = 0; i < TEST_MATRIX_MAJOR_DIM; ++i)
    {
        X[i] = fRNG_float_range(&rng, -10, 10);
        jmtx_scalar_t val[TEST_MATRIX_MINOR_DIM];
