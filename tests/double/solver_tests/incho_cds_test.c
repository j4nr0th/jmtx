...
int main()
{
    ...
    MATRIX_TEST_CALL(jmtxds_matrix_cds_new(&mtx, PROBLEM_INTERNAL_PTS, PROBLEM_INTERNAL_PTS, 0, (const int32_t[]){0}, NULL));
    ...
    jmtxd_matrix_cds* cholesky = NULL;
    const double t0_decomp = omp_get_wtime();
    MATRIX_TEST_CALL(jmtxd_incomplete_cholesky_cds(mtx, &cholesky, NULL));
    ...
    jmtxd_matrix_cds* cho_t = NULL;
    MATRIX_TEST_CALL(jmtxd_matrix_cds_transpose(cholesky, &cho_t, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    print_cdsd_matrix(cho_t);

    jmtxd_matrix_cds* approx_mtx = NULL;
    MATRIX_TEST_CALL(jmtxd_matrix_multiply_cds(cholesky, cho_t, &approx_mtx, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
