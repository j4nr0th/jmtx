// Automatically generated from tests/float/matrix_ops_tests/crs_addition_test.c on Fri Dec  1 06:43:09 2023
//
...
int main()
{
    ...
    jmtx_result mtx_res;
    MATRIX_TEST_CALL(jmtxds_matrix_crs_new(&mtx, 4, 4, 4, NULL));
    ...
    const double m3[16] = {1, 0, 0, 1, 0, 2, 3, 0, 0, 3, 2, 0, 1, 0, 0, 1};
    int beef_status;
    ...
    for (uint32_t i = 0; i < 4; ++i)
    {
        for (uint32_t j = 0; j < 4; ++j)
        {
            ...
            mtx_res = jmtxds_matrix_crs_add_to_entry(mtx, i, j, v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        }
    }
    ...
    for (uint32_t i = 0; i < 4; ++i)
    {
        for (uint32_t j = 0; j < 4; ++j)
        {
            double x = m1[j + i * 4];
            double v;
            mtx_res = jmtxds_matrix_crs_get_entry(mtx, i, j, &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == x);
        }
    }
    ...
    for (uint32_t i = 0; i < 4; ++i)
    {
        for (uint32_t j = 0; j < 4; ++j)
        {
            double x = m1[j + i * 4] + m2[j + i * 4] + m3[j + i * 4];
            double v;
            mtx_res = jmtxds_matrix_crs_get_entry(mtx, i, j, &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == x);
    ...
    //  Removing 2nd column
    MATRIX_TEST_CALL(jmtxds_matrix_crs_remove_column(mtx, 1));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    for (uint32_t i = 0; i < 4; ++i)
    {
        for (uint32_t j = 0; j < 3; ++j)
        {
            ...
            double x = m1[c + i * 4] + m2[c + i * 4] + m3[c + i * 4];
            double v;
            mtx_res = jmtxds_matrix_crs_get_entry(mtx, i, j, &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == x);
    ...
    //  Removing 1st row
    MATRIX_TEST_CALL(jmtxds_matrix_crs_remove_row(mtx, 0));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    for (uint32_t i = 1; i < 4; ++i)
    {
        for (uint32_t j = 0; j < 3; ++j)
        {
            ...
            double x = m1[c + i * 4] + m2[c + i * 4] + m3[c + i * 4];
            double v;
            mtx_res = jmtxds_matrix_crs_get_entry(mtx, i - 1, j, &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == x);
        }
    }
