...
static int are_value_arrays_the_same(unsigned len, const double a1[JMTX_ARRAY_ATTRIB(const static len)],
                                     const double a2[JMTX_ARRAY_ATTRIB(const static len)])
{
...
static int are_index_arrays_the_same(unsigned len, const uint32_t a1[JMTX_ARRAY_ATTRIB(const static len)],
                                     const uint32_t a2[JMTX_ARRAY_ATTRIB(const static len)])
{
    for (unsigned i = 0; i < len; ++i)
    {
        if (a1[i] != a2[i])
...
int test_row(void)
{
    ...
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_row(test_matrix, 0, 3, (double[]){1.0f, 2.0f, 3.0f}, (uint32_t[]){0, 1, 2}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the second row
    {
        MATRIX_TEST_CALL(jmtxds_matrix_cds_set_row(test_matrix, 1, 4, (double[]){-1.0f, -2.0f, -3.0f, -4.0f},
                                                   (uint32_t[]){0, 1, 2, 3}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    ...
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_row(test_matrix, 2, 4, (double[]){1.0f, 2.0f, 3.0f, 4.0f}, (uint32_t[]){1, 2, 3, 4}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the fourth row
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_row(test_matrix, 3, 3, (double[]){-5.0f, -6.0f, -7.0f}, (uint32_t[]){2, 3, 4}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the fifth row
...
int test_col(void)
{
    ...
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_col(test_matrix, 0, 3, (double[]){1.0f, 2.0f, 3.0f}, (uint32_t[]){0, 1, 2}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the second col
    {
        MATRIX_TEST_CALL(jmtxds_matrix_cds_set_col(test_matrix, 1, 4, (double[]){-1.0f, -2.0f, -3.0f, -4.0f},
                                                   (uint32_t[]){0, 1, 2, 3}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    ...
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_col(test_matrix, 2, 4, (double[]){1.0f, 2.0f, 3.0f, 4.0f}, (uint32_t[]){1, 2, 3, 4}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the fourth col
    {
        MATRIX_TEST_CALL(
            jmtxds_matrix_cds_set_col(test_matrix, 3, 3, (double[]){-5.0f, -6.0f, -7.0f}, (uint32_t[]){2, 3, 4}));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    print_cdsd_matrix(test_matrix);
    //  Set the fifth col
