...
int main()
{
    ...
    for (uint32_t row = 0; row < n_rows; ++row)
    {
        ...
        {
            float v;
            mtx_res = jmtx_matrix_ccs_get_entry(mtx, row, col, &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == 0.0f);
//            printf("%7g ", (double)v);
        }
//        printf("\n");
    ...
    MATRIX_TEST_CALL(jmtx_matrix_ccs_get_entry(mtx, n_rows, n_cols - 1, &v));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_get_entry(mtx, n_rows - 1, n_cols, &v));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_get_entry(mtx, n_rows, n_cols, &v));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_get_entry(mtx, 1241284124, 1212412, &v));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_get_entry(mtx, 0, -1, &v));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_set_entry(mtx, n_rows, n_cols - 1, 6.9f));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_set_entry(mtx, n_rows - 1, n_cols, 420.0f));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_set_entry(mtx, n_rows, n_cols, 420.0f));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_set_entry(mtx, 1241284124, 1212412, 420.0f));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    MATRIX_TEST_CALL(jmtx_matrix_ccs_set_entry(mtx, 0, -1, 420.0f));
    ASSERT(mtx_res == JMTX_RESULT_INDEX_OUT_OF_BOUNDS);
    ...
    {
        mtx_res = jmtx_matrix_ccs_set_entry(mtx, i, i, -0.0f);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        mtx_res = jmtx_matrix_ccs_beef_check(mtx, &beef_status);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(beef_status == 0x0000Beef);
...
//        print_ccs_matrix(&mtx);
        mtx_res = jmtx_matrix_ccs_set_entry(mtx, row_indices[i], col_indices[i], other_values[i]);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        mtx_res = jmtx_matrix_ccs_beef_check(mtx, &beef_status);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(beef_status == 0x0000Beef);
...
//        print_ccs_matrix(&mtx);
        ...
        {
            mtx_res = jmtx_matrix_ccs_get_entry(mtx, row_indices[j], col_indices[j], &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
...
//    print_ccs_matrix(&mtx);
    ...
    for (uint32_t i = 0; i < 16; ++i)
    {
        mtx_res = jmtx_matrix_ccs_get_entry(mtx, row_indices[i], col_indices[i], &v);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    for (uint32_t i = 0; i < 16; ++i)
    {
        mtx_res = jmtx_matrix_ccs_set_entry(mtx, col_indices[i], row_indices[i], other_values[i]);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        mtx_res = jmtx_matrix_ccs_beef_check(mtx, &beef_status);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(beef_status == 0x0000Beef);
        ...
        for (uint32_t j = 0; j <= i; ++j)
        {
            mtx_res = jmtx_matrix_ccs_get_entry(mtx, col_indices[j], row_indices[j], &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == other_values[j]);
        }
    }
//    print_ccs_matrix(&mtx);
    ...
    for (uint32_t i = 0; i < 16; ++i)
    {
        mtx_res = jmtx_matrix_ccs_get_entry(mtx, col_indices[i], row_indices[i], &v);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(v == other_values[i]);
    }
