// Automatically generated from tests/cfloat/solver_tests/incho_test.c on Fri Dec  1 18:48:10 2023
// Automatically generated from tests/cdouble/solver_tests/incho_test.c on Fri Dec  1 17:35:45 2023
//
...
int main()
{
    ...
    jmtxz_matrix_crs* cholesky = NULL;
    const double t0_decomp = omp_get_wtime();
    MATRIX_TEST_CALL(jmtxz_decompose_icho_crs(mtx, &cholesky, NULL));
    const double t1_decomp = omp_get_wtime();
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    print_crsz_matrix(cholesky);
    printf("Decomposition took %g seconds and the result: %s\n", t1_decomp -
    t0_decomp, jmtx_result_to_str(mtx_res));

    jmtxz_matrix_crs* cpy;
    jmtxz_matrix_ccs* cho_t = NULL;
    MATRIX_TEST_CALL(jmtxz_matrix_crs_copy(cholesky, &cpy, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    cho_t = (jmtxz_convert_crs_to_ccs_inplace_transpose(cpy));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    print_ccsz_matrix(cho_t);

    jmtxz_matrix_crs* approx_mtx = NULL;
    MATRIX_TEST_CALL(jmtxz_multiply_matrix_crs(cholesky, cho_t, &approx_mtx, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    print_crsz_matrix(mtx);
    print_crsz_matrix(approx_mtx);

    MATRIX_TEST_CALL(jmtxzs_matrix_crs_destroy(approx_mtx));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    MATRIX_TEST_CALL(jmtxzs_matrix_ccs_destroy(cho_t));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    MATRIX_TEST_CALL(jmtxzs_matrix_crs_destroy(cholesky));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
