// Automatically generated from tests/double/solver_tests/omp_cg_test.c on Fri Dec  1 06:43:09 2023
//
...
int main()
{
    ...
    _Complex double* const exact_solution = calloc(sizeof(*exact_solution), PROBLEM_DIMS); // exact solution of u
    ASSERT(exact_solution);
    _Complex double* const forcing_vector = calloc(sizeof(*forcing_vector), PROBLEM_DIMS); // forcing vector for u (all values are 1)
    ASSERT(forcing_vector);
    _Complex double* const iterative_solution = calloc(sizeof(*iterative_solution), PROBLEM_DIMS);
    ASSERT(iterative_solution);
    _Complex double* const aux_v1 = calloc(sizeof(*aux_v1), RESTART_INTERVAL);
    ASSERT(aux_v1);
    _Complex double* const aux_v2 = calloc(sizeof(*aux_v2), RESTART_INTERVAL);
    ASSERT(aux_v2);
    _Complex double* const aux_v3 = calloc(sizeof(*aux_v3), RESTART_INTERVAL);
    ASSERT(aux_v3);
    _Complex double* const aux_v4 = calloc(sizeof(*aux_v4), RESTART_INTERVAL);
    ASSERT(aux_v4);
    _Complex double* const aux_v5 = calloc(sizeof(*aux_v5), RESTART_INTERVAL);
    ASSERT(aux_v5);
    _Complex double* const aux_vecs = calloc(sizeof(*aux_vecs), RESTART_INTERVAL * PROBLEM_DIMS);
    ASSERT(aux_vecs);
...
//    print_crsd_matrix(mtx);
...

    {
        ...
        mtx_res = jmtxz_solve_iterative_gmresm_crs(
