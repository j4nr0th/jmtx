// Automatically generated from tests/double/solver_tests/bicgstab_cds_test.c on Sun Dec 17 16:07:06 2023
//
...
#include "../../../include/jmtx/cdouble/solvers/recursive_generalized_minimum_residual_iteration.h"
...
int main()
{
    ...
    _Complex double* const exact_solution = calloc(PROBLEM_DIMS, sizeof(*exact_solution)); // exact solution of u
    ASSERT(exact_solution);
    _Complex double* const forcing_vector = calloc(PROBLEM_DIMS, sizeof(*forcing_vector)); // forcing vector for u (all values are 1)
    ASSERT(forcing_vector);
    _Complex double* const iterative_solution = calloc(PROBLEM_DIMS, sizeof(*iterative_solution));
    ASSERT(iterative_solution);
    _Complex double* const aux_v1 = calloc(RESTART_INTERVAL, sizeof(*aux_v1));
    ASSERT(aux_v1);
    _Complex double* const aux_v2 = calloc(RESTART_INTERVAL, sizeof(*aux_v2));
    ASSERT(aux_v2);
    _Complex double* const aux_v3 = calloc(RESTART_INTERVAL, sizeof(*aux_v3));
    ASSERT(aux_v3);
    _Complex double* const aux_v4 = calloc(RESTART_INTERVAL, sizeof(*aux_v4));
    ASSERT(aux_v4);
    _Complex double* const aux_v5 = calloc(RESTART_INTERVAL, sizeof(*aux_v5));
    ASSERT(aux_v5);
    _Complex double* const aux_v6 = calloc(PROBLEM_DIMS, sizeof(*aux_v6));
    ASSERT(aux_v6);
    _Complex double* const aux_v7 = calloc(RESTART_INTERVAL, sizeof(*aux_v7));
    ASSERT(aux_v7);
    _Complex double* const aux_vecs = calloc(PROBLEM_DIMS * RESTART_INTERVAL, sizeof(*aux_vecs));
    ASSERT(aux_vecs);
    _Complex double* const aux_vecs2 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs2));
    ASSERT(aux_vecs2);
    _Complex double* const aux_vecs3 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs3));
    ASSERT(aux_vecs3);
    ...
    MATRIX_TEST_CALL(jmtxzs_matrix_cds_new(&mtx, PROBLEM_DIMS - 2, PROBLEM_DIMS - 2, 3, (int32_t[]){-1, 0, +1}, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    //  Build the matrix
    uint32_t len;
    _Complex double* dia = jmtxz_matrix_cds_allocate_diagonal(mtx, 0, &len);
    //  Main diagonal
    for (uint_fast32_t i = 0; i < len; ++i)
    {
        dia[i] = -2.0f / (dx * dx);
    }
    dia = jmtxz_matrix_cds_allocate_diagonal(mtx, +1, &len);
    //  Superdiagonal
    for (uint_fast32_t i = 0; i < len; ++i)
    {
    ...
    jmtxz_matrix_brm* r_mtx;
    MATRIX_TEST_CALL(jmtxz_matrix_brm_new(&r_mtx, RESTART_INTERVAL, RESTART_INTERVAL, RESTART_INTERVAL - 1, 0, NULL, NULL));
...
//    print_cdsz_matrix(mtx);
...

    {
        ...
        mtx_res = jmtxz_solve_iterative_gmresr_cds(
...
//        }
    ...
    jmtxz_matrix_brm_destroy(r_mtx);
