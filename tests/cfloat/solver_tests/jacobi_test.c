...
int main()
{
    ...
    MATRIX_TEST_CALL(jmtxc_solve_iterative_jacobi_crs(matrix, f_exact, y_approx, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    ...
    MATRIX_TEST_CALL(jmtxc_solve_iterative_jacobi_relaxed_crs(matrix, f_exact, y_relax, relax_factor, x_aux1, x_aux2,
                                                              &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    printf("After %" PRIu32 " iterations, the final error was %g\n", solver_arguments.out_last_iteration,
           solver_arguments.out_last_error);
    ...
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts_0);
    jmtxc_solve_iterative_jacobi_relaxed_crs(matrix, f_exact, y_relax, relax_factor, x_aux1, x_aux2, &solver_arguments);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts_1);
