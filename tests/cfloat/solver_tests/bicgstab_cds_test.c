...
int main()
{
    ...
    for (unsigned i = 0; i < CG_ITERATION_ROUND; ++i)
    {
        ...
        mtx_res =
            jmtxcs_solve_iterative_bicgstab_cds(mtx, PROBLEM_DIMS - 2, forcing_vector + 1, iterative_solution + 1,
                                                aux_v1, aux_v2, aux_v3, aux_v4, aux_v5, aux_v6, &solver_arguments);
