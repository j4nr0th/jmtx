...
//            ASSERT(beef_stat == 0xBeef);
    ...
    {
        MATRIX_TEST_CALL(jmtxcs_matrix_crs_new(mtx_array + i, PROBLEM_INTERNAL_PTS, (sizes[i + 1] - sizes[i]), 5 * (sizes[i + 1] - sizes[i]) < WORK_DIVISIONS * (sizes[i + 1] - sizes[i]) ?: WORK_DIVISIONS * (sizes[i + 1] - sizes[i]), NULL));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
