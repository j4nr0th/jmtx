// Automatically generated from tests/float/matrix_ops_tests/omp_build_test.c on Fri Dec  1 17:35:45 2023
//
...
static void construct_rows_of_matrix(jmtxc_matrix_crs* mtx, unsigned first, unsigned count, const float rdy2, const float rdx2)
{
    unsigned i, j;
...
//        MATRIX_TEST_CALL(
        jmtxc_matrix_crs_build_row(mtx, k - first, l, positions, values);
//                );
//        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
//        int beef_stat;
//        ASSERT(mtx_res == jmtxcs_matrix_crs_beef_check(mtx, &beef_stat));
...
int main()
{
    ...
    MATRIX_TEST_CALL(jmtxcs_matrix_crs_new(&mtx, PROBLEM_INTERNAL_PTS, PROBLEM_INTERNAL_PTS, 5 * PROBLEM_INTERNAL_PTS > PROBLEM_INTERNAL_PTS * PROBLEM_INTERNAL_PTS ?: PROBLEM_INTERNAL_PTS * PROBLEM_INTERNAL_PTS, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    //  Serial construction
...
//            ASSERT(beef_stat == 0xBeef);
    ...
    jmtxc_matrix_crs** const mtx_array = calloc(WORK_DIVISIONS, sizeof(*mtx_array));
    ASSERT(mtx_array);
    ...
    {
        MATRIX_TEST_CALL(jmtxcs_matrix_crs_new(mtx_array + i, (sizes[i + 1] - sizes[i]), PROBLEM_INTERNAL_PTS, 5 * (sizes[i + 1] - sizes[i]) < WORK_DIVISIONS * (sizes[i + 1] - sizes[i]) ?: WORK_DIVISIONS * (sizes[i + 1] - sizes[i]),  NULL));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
...
#pragma omp task default(none) shared(rdx2, rdy2, sizes, mtx_array) firstprivate(i)
    ...
    jmtxc_matrix_crs* joined;
    MATRIX_TEST_CALL(jmtxcs_matrix_crs_join_vertically(&joined, NULL, WORK_DIVISIONS,
                                                     (const jmtxc_matrix_crs**) mtx_array));
    ...
    for (unsigned i = 0; i < WORK_DIVISIONS; ++i)
    {
        MATRIX_TEST_CALL(jmtxcs_matrix_crs_destroy(mtx_array[i]));
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    }
    ...
    for (unsigned i = 0; i < PROBLEM_INTERNAL_PTS; ++i)
    {
        ...
        {
            _Complex float v_serial, v_joined;
            ASSERT(jmtxcs_matrix_crs_get_entry(mtx, i, j, &v_serial) == JMTX_RESULT_SUCCESS);
            ASSERT(jmtxcs_matrix_crs_get_entry(joined, i, j, &v_joined) == JMTX_RESULT_SUCCESS);
            ASSERT(v_serial == v_joined);
        }
    }
    MATRIX_TEST_CALL(jmtxcs_matrix_crs_destroy(joined));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
