...
#pragma omp parallel for shared(exact_solution) default(none) schedule(static)
    ...
    //  Build the matrix
    {
        ...
        const JMTX_SCALAR_T values1[2] = {2.0f / dx2, -1.0f / dx2};
        const uint32_t indices2[2] = {PROBLEM_DIMS - 4, PROBLEM_DIMS - 3};
        const JMTX_SCALAR_T values2[2] = {-1.0f / dx2, 2.0f / dx2};
        forcing_vector[0] = 0.0f;
        forcing_vector[PROBLEM_DIMS - 1] = 0.0f;
        ASSERT(JMTX_RESULT_SUCCESS == (JMTX_NAME_TYPED(matrix_crs_set_row)(mtx, 0, 2, indices1, values1)));
        ASSERT(JMTX_RESULT_SUCCESS ==
               (JMTX_NAME_TYPED(matrix_crs_set_row)(mtx, PROBLEM_DIMS - 3, 2, indices2, values2)));
    }
    for (unsigned i = 1; i < PROBLEM_DIMS - 3; ++i)
    {
        const uint32_t indices[3] = {i - 1, i, i + 1};
        const JMTX_SCALAR_T values[3] = {-1.0f / dx2, 2.0f / dx2, -1.0f / dx2};
        ASSERT(JMTX_RESULT_SUCCESS == (JMTX_NAME_TYPED(matrix_crs_set_row)(mtx, i, 3, indices, values)));
    }
...
#pragma omp parallel for shared(forcing_vector, mtx, exact_solution) default(none) schedule(static)
    ...
    {
        forcing_vector[i + 1] = JMTX_NAME_TYPED(matrix_crs_vector_multiply_row)(mtx, exact_solution + 1, i);
    }
    JMTX_NAME_TYPED(matrix_crs) *cho = NULL;
    MATRIX_TEST_CALL(JMTX_NAME_TYPED(decompose_icho_crs)(mtx, &cho, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    //    print_crsd_matrix(cho);

    JMTX_NAME_TYPED(matrix_crs) * cho_t;
    MATRIX_TEST_CALL(JMTX_NAME_TYPED(matrix_crs_transpose)(cho, &cho_t, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    for (unsigned i = 0; i < CG_ITERATION_ROUND; ++i)
    {
        ...
        mtx_res = JMTX_NAME_TYPED(incomplete_cholesky_preconditioned_solve_iterative_conjugate_gradient_crs)(
    ...
    {
        const double x = (double)i / (double)(PROBLEM_DIMS - 1);
        printf("|u_ex(%g) - u_num(%g)| = %e\n", x, x, JMTX_ABS(exact_solution[i] - iterative_solution[i]));
    }

    JMTX_NAME_TYPED(matrix_crs_destroy)(cho_t);
    JMTX_NAME_TYPED(matrix_crs_destroy)(cho);
