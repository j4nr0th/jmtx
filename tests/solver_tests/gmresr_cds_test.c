...
enum
{
    ...
    RESTART_INTERVAL = (1 << 4),
    TRUNCATION_INTERVAL = (1 << 4)
};
...
int main()
{
    ...
    double *const aux_v1 = calloc(RESTART_INTERVAL, sizeof(*aux_v1));
    ASSERT(aux_v1);
    double *const aux_v2 = calloc(RESTART_INTERVAL, sizeof(*aux_v2));
    ASSERT(aux_v2);
    double *const aux_v3 = calloc(RESTART_INTERVAL, sizeof(*aux_v3));
    ASSERT(aux_v3);
    double *const aux_v4 = calloc(RESTART_INTERVAL, sizeof(*aux_v4));
    ASSERT(aux_v4);
    double *const aux_v5 = calloc(RESTART_INTERVAL, sizeof(*aux_v5));
    ASSERT(aux_v5);
    double *const aux_v6 = calloc(PROBLEM_DIMS, sizeof(*aux_v6));
    ASSERT(aux_v6);
    double *const aux_v7 = calloc(RESTART_INTERVAL, sizeof(*aux_v7));
    ASSERT(aux_v7);
    double *const aux_vecs = calloc(PROBLEM_DIMS * RESTART_INTERVAL, sizeof(*aux_vecs));
    ASSERT(aux_vecs);
    double *const aux_vecs2 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs2));
    ASSERT(aux_vecs2);
    double *const aux_vecs3 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs3));
    ASSERT(aux_vecs3);
    ...
    MATRIX_TEST_CALL(
        jmtxd_matrix_brm_new(&r_mtx, RESTART_INTERVAL, RESTART_INTERVAL, RESTART_INTERVAL - 1, 0, NULL, NULL));
...

    {
        ...
        mtx_res = jmtxd_solve_iterative_gmresr_cds(mtx, forcing_vector + 1, iterative_solution + 1, RESTART_INTERVAL,
                                                   TRUNCATION_INTERVAL, r_mtx, aux_v1, aux_v2, aux_v3, aux_v4, aux_v5,
                                                   aux_v6, aux_vecs, aux_vecs2, aux_vecs3, &solver_arguments);
    ...
    jmtxd_matrix_brm_destroy(r_mtx);
