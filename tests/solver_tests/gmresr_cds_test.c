...
#include "solvers/recursive_generalized_minimum_residual_iteration.h"
...
int main()
{
    ...
    ASSERT(aux_vecs);
    JMTX_SCALAR_T *const aux_vecs2 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs2));
    ASSERT(aux_vecs2);
    JMTX_SCALAR_T *const aux_vecs3 = calloc(PROBLEM_DIMS * TRUNCATION_INTERVAL, sizeof(*aux_vecs3));
    ASSERT(aux_vecs3);
...

    {
        ...
        mtx_res = JMTX_NAME_TYPED(solve_iterative_gmresr_cds)(
            mtx, forcing_vector + 1, iterative_solution + 1, RESTART_INTERVAL, TRUNCATION_INTERVAL, r_mtx, aux_v1,
            aux_v2, aux_v3, aux_v4, aux_v5, aux_v6, aux_vecs, aux_vecs2, aux_vecs3, &solver_arguments);
    ...
    for (unsigned i = 0; i < PROBLEM_DIMS; ++i)
    {
        ...
        const JMTX_SCALAR_T local_error = exact_solution[i] - iterative_solution[i];
        rms_error += JMTX_DOT(local_error, local_error);
    }
