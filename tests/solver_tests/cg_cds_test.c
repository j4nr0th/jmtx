...
int main()
{
    ...
    solver_arguments = {
        .in_convergence_criterion = 1e-4f, .in_max_iterations = MAX_ITERATIONS,
        //            .opt_error_evolution = err_evol,
    };
    for (unsigned i = 0; i < CG_ITERATION_ROUND; ++i)
    {
        const double t0 = omp_get_wtime();
        mtx_res = JMTX_NAME_TYPED(solve_iterative_conjugate_gradient_cds)(mtx, forcing_vector, iterative_solution + 1,
