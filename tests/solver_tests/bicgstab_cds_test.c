...
int main()
{
    ...
    const JMTX_REAL_T dx = 1.0f / (JMTX_REAL_T)(PROBLEM_DIMS - 1);
    ...
    for (unsigned i = 0; i < CG_ITERATION_ROUND; ++i)
    {
        ...
        mtx_res =
            JMTX_NAME_TYPED(solve_iterative_bicgstab_cds)(mtx, forcing_vector + 1, iterative_solution + 1, aux_v1,
                                                          aux_v2, aux_v3, aux_v4, aux_v5, aux_v6, &solver_arguments);
        ...
        {
            const JMTX_SCALAR_T local_error = (exact_solution[j] - iterative_solution[j]);
