...
#include "../../../source/matrices/band_row_major_safe.h"
...
int main()
{
    ...
    MATRIX_TEST_CALL(jmtxd_multiply_matrix_brm(lower_brm, upper_brm, &multiplied_brm, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    for (unsigned i = 0; i < PROBLEM_SIZE; ++i)
    {
        ...
        for (unsigned j = 0; j < PROBLEM_SIZE; ++j)
        {
            ASSERT(are_close(jmtxd_matrix_crs_get_entry(multiplied, i, j), exact_multiplied[i][j], default_r_tol,
                             default_r_tol));
            ASSERT(are_close(jmtxd_matrix_brm_get_entry(multiplied_brm, i, j), exact_multiplied[i][j], default_r_tol,
                             default_r_tol));
        }
    }
    ...
    MATRIX_TEST_CALL(jmtxd_decompose_lu_brm(combined_brm, &dl, &du, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    print_crsd_matrix(upper);
    ...
    for (unsigned i = 0; i < PROBLEM_SIZE; ++i)
    {
        for (unsigned j = 0; j < PROBLEM_SIZE; ++j)
        {
            ASSERT(are_close(jmtxd_matrix_brm_get_entry(du, i, j), jmtxd_matrix_brm_get_entry(upper_brm, i, j),
                             default_r_tol, default_r_tol));
            ASSERT(are_close(jmtxd_matrix_brm_get_entry(dl, i, j), jmtxd_matrix_brm_get_entry(lower_brm, i, j),
                             default_r_tol, default_r_tol));
        }
    }
    ...
    jmtxd_solve_direct_lu_brm(lower_brm, upper_brm, yb, xb);
