...
int main()
{
    ...
    JMTX_SCALAR_T aux_v1[PROBLEM_DIMS], aux_v2[PROBLEM_DIMS];
...
#pragma omp parallel for shared(forcing_vector) default(none) schedule(static)
    ...
    mtx_res = JMTX_NAME_TYPED(solve_iterative_jacobi_crs_parallel)(mtx, forcing_vector, iterative_solution + 1, aux_v1,
                                                                   aux_v2, &solver_arguments);
