...
//    print_crs_matrix(&mtx);
    ...
    for (uint32_t i = 0; i < 16; ++i)
    {
        mtx_res = matrix_crs_set_element(&mtx, col_indices[i], row_indices[i], other_values[i]);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        mtx_res = matrix_crs_beef_check(&mtx, &beef_status);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(beef_status == 0x0000Beef);
        ...
        for (uint32_t j = 0; j <= i; ++j)
        {
            mtx_res = matrix_crs_get_element(&mtx, col_indices[j], row_indices[j], &v);
            ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
            ASSERT(v == other_values[j]);
        }
    }
//    print_crs_matrix(&mtx);
    ...
    for (uint32_t i = 0; i < 16; ++i)
    {
        mtx_res = matrix_crs_get_element(&mtx, col_indices[i], row_indices[i], &v);
        ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
        ASSERT(v == other_values[i]);
    }
