...
int main()
{
    ...
    for (unsigned i = 0; i < INTERNAL_SIZE_Y; ++i)
    {
        for (unsigned j = 0; j < INTERNAL_SIZE_X; ++j)
        {
            ...
            jmtx_matrix_cds_set_row(mtx, lexicographic_position(i, j), k, values, positions);
        }
    }
    print_cds_matrix(mtx);
    jmtx_matrix_cds* cholesky = NULL;
    const double t0_decomp = omp_get_wtime();
    MATRIX_TEST_CALL(jmtx_incomplete_cholesky_cds(mtx, &cholesky, NULL));
    ...
    MATRIX_TEST_CALL(jmtx_matrix_multiply_cds(cholesky, cho_t, &approx_mtx, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    print_cds_matrix(mtx);
    print_cds_matrix(approx_mtx);

    MATRIX_TEST_CALL(jmtxs_matrix_cds_destroy(approx_mtx));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    MATRIX_TEST_CALL(jmtxs_matrix_cds_destroy(cho_t));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    MATRIX_TEST_CALL(jmtxs_matrix_cds_destroy(cholesky));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
