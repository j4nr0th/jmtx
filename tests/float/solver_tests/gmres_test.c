...
int main()
{
    ...
    float *const exact_solution = calloc(sizeof(*exact_solution), PROBLEM_DIMS); // exact solution of u
    ...
    float *const iterative_solution = calloc(sizeof(*iterative_solution), PROBLEM_DIMS);
    ASSERT(iterative_solution);
    float *const aux_v1 = calloc(sizeof(*aux_v1), RESTART_INTERVAL);
    ASSERT(aux_v1);
    float *const aux_v2 = calloc(sizeof(*aux_v2), RESTART_INTERVAL);
    ASSERT(aux_v2);
    float *const aux_v3 = calloc(sizeof(*aux_v3), RESTART_INTERVAL);
    ASSERT(aux_v3);
    float *const aux_v4 = calloc(sizeof(*aux_v4), RESTART_INTERVAL);
    ASSERT(aux_v4);
    float *const aux_v5 = calloc(sizeof(*aux_v5), RESTART_INTERVAL);
    ASSERT(aux_v5);
    float *const aux_vecs = calloc(sizeof(*aux_vecs), RESTART_INTERVAL * PROBLEM_DIMS);
    ASSERT(aux_vecs);
    ...
    jmtxf_matrix_brm *r_mtx = NULL;
    MATRIX_TEST_CALL(
        jmtxf_matrix_brm_new(&r_mtx, RESTART_INTERVAL, RESTART_INTERVAL, RESTART_INTERVAL - 1, 0, NULL, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
...

    {
        ...
        mtx_res = jmtx_solve_iterative_gmresm_crs(mtx, forcing_vector, iterative_solution + 1, RESTART_INTERVAL, r_mtx,
                                                  aux_v1, aux_v2, aux_v3, aux_v4, aux_v5, aux_vecs, &solver_arguments);
    ...
    jmtxf_matrix_brm_destroy(r_mtx);
