...
int main()
{
    ...
    static float x_aux1[steps] = {0};
    static float x_aux2[steps] = {0};
    ...
    f_exact[0] = 0.0f;
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx10, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx20, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx30, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx40, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);

    printf("Using normal Jacobi iteration\n");
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx, x_aux1, x_aux2, &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    printf("After %" PRIu32 " iterations, the final error was %g\n", solver_arguments.out_last_iteration,
    ...
    jmtx_solve_iterative_jacobi_crs(matrix, f_exact, y_approx, x_aux1, x_aux2, &solver_arguments);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts_1);
    printf("Time needed for jmtx_solve_iterative_jacobi_crs to solve the problem: %g ms\n",
           1e3 * ts_difference(&ts_0, &ts_1));

    const float relax_factor = 1.0f;
    printf("Using a relaxation factor of %g\n", relax_factor);
    MATRIX_TEST_CALL(jmtx_solve_iterative_jacobi_relaxed_crs(matrix, f_exact, y_relax, relax_factor, x_aux1, x_aux2,
                                                             &solver_arguments));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS || mtx_res == JMTX_RESULT_NOT_CONVERGED);
    printf("After %" PRIu32 " iterations, the final error was %g\n", solver_arguments.out_last_iteration,
           solver_arguments.out_last_error);
    ...
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts_0);
    jmtx_solve_iterative_jacobi_relaxed_crs(matrix, f_exact, y_relax, relax_factor, x_aux1, x_aux2, &solver_arguments);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts_1);
    printf("Time needed for jmtx_solve_iterative_jacobi_relaxed_crs to solve the problem: %g ms\n",
           1e3 * ts_difference(&ts_0, &ts_1));
    ...
    ASSERT(sfg_exact);
    cplot_curve2d *exact_plot =
        cplot_plot(cplot_ctx, sfg_exact, steps, x_values, y_exact, (cplot_color){.b = 0xFF, .a = 0xFF}, "Exact");
    ASSERT(exact_plot);
    ...
    cplot_curve2d *approx10_plot = cplot_plot(cplot_ctx, sfg_apporx, steps, x_values, y_approx10,
                                              (cplot_color){.b = 255, .r = 0, .a = 0xFF}, "20 %");
    ASSERT(approx10_plot);

    cplot_curve2d *approx20_plot = cplot_plot(cplot_ctx, sfg_apporx, steps, x_values, y_approx20,
                                              (cplot_color){.b = 215, .r = 40, .a = 0xFF}, "40 %");
    ASSERT(approx20_plot);

    cplot_curve2d *approx30_plot = cplot_plot(cplot_ctx, sfg_apporx, steps, x_values, y_approx30,
                                              (cplot_color){.b = 175, .r = 80, .a = 0xFF}, "60 %");
    ASSERT(approx30_plot);

    cplot_curve2d *approx40_plot = cplot_plot(cplot_ctx, sfg_apporx, steps, x_values, y_approx40,
                                              (cplot_color){.b = 135, .r = 120, .a = 0xFF}, "80 %");
    ASSERT(approx40_plot);

    char name_buffer[64];
    snprintf(name_buffer, sizeof(name_buffer), "Converged: %d", iter_count);
    cplot_curve2d *approx_plot =
        cplot_plot(cplot_ctx, sfg_apporx, steps, x_values, y_approx, (cplot_color){.r = 0xFF, .a = 0xFF}, name_buffer);
    ASSERT(approx_plot);
    ...
    cplot_curve2d *err_plot = cplot_plot(cplot_ctx, sfg_error, iter_count, iter_count_array, error_evol,
                                         (cplot_color){.g = 0xFF, .a = 0xFF}, "Regular");
    ASSERT(err_plot);
    cplot_curve2d *rel_plot = cplot_plot(cplot_ctx, sfg_error, iter_relax, iter_count_array, error_relaxed,
