...
int main()
{
    ...
    jmtx_result mtx_res;
    jmtx_matrix_drm* mtx, *decomp, *decompnp;
    MATRIX_TEST_CALL(jmtx_matrix_drm_new(&mtx, PROBLEM_DIMS, PROBLEM_DIMS, NULL, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    MATRIX_TEST_CALL(jmtx_matrix_drm_new(&decomp, PROBLEM_DIMS, PROBLEM_DIMS, NULL, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    MATRIX_TEST_CALL(jmtx_matrix_drm_new(&decompnp, PROBLEM_DIMS, PROBLEM_DIMS, NULL, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);

    jmtx_matrix_drm_set_row(mtx, 0, (float[PROBLEM_DIMS]){ 1, 3, 0, 0});
    jmtx_matrix_drm_set_row(mtx, 1, (float[PROBLEM_DIMS]){-.001f, 2, 40, 0});
    jmtx_matrix_drm_set_row(mtx, 2, (float[PROBLEM_DIMS]){ 0, 1, 1, 0});
    jmtx_matrix_drm_set_row(mtx, 3, (float[PROBLEM_DIMS]){ 0, 200,-400, 1});
    print_drm_matrix(mtx);
    uint32_t ubw, lbw;
    jmtx_matrix_drm_get_bandwidths(mtx, &ubw, &lbw);
    ...
    jmtx_matrix_drm_vector_multiply(mtx, test_vector_in1, test_vector_out1);


    clock_gettime(CLOCK_MONOTONIC, &ts0);
    jmtx_decompose_lu_pivot_drm(mtx, decomp);
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts0);
    jmtx_decompose_lu_drm(mtx, decompnp);
    clock_gettime(CLOCK_MONOTONIC, &ts1);
    ...
    jmtx_matrix_drm_destroy(decompnp);
    jmtx_matrix_drm_destroy(decomp);
    jmtx_matrix_drm_destroy(mtx);
