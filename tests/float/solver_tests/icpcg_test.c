...
#pragma omp parallel for shared(exact_solution) default(none) schedule(static)
    ...
    //  Build the matrix
    {
        ...
        const float values1[2] = {2.0f / dx2, -1.0f / dx2};
        const uint32_t indices2[2] = {PROBLEM_DIMS - 4, PROBLEM_DIMS - 3};
        const float values2[2] = {-1.0f / dx2, 2.0f / dx2};
        forcing_vector[0] = 0.0f;
        forcing_vector[PROBLEM_DIMS - 1] = 0.0f;
        ASSERT(JMTX_RESULT_SUCCESS == (jmtxs_matrix_crs_set_row(mtx, 0, 2, indices1, values1)));
        ASSERT(JMTX_RESULT_SUCCESS == (jmtxs_matrix_crs_set_row(mtx, PROBLEM_DIMS - 3, 2, indices2, values2)));
    }
...
#pragma omp parallel for shared(forcing_vector, mtx, exact_solution) default(none) schedule(static)
    ...
    MATRIX_TEST_CALL(jmtxs_matrix_crs_destroy(cho_t));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    MATRIX_TEST_CALL(jmtxs_matrix_crs_destroy(cho));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
