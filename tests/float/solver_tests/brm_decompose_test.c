...
static float get_rms_error_for_element_count(const unsigned n)
{
    ...
    jmtxf_matrix_crs *system_matrix;
    jmtx_result mtx_res;
    ...
    //  Build the matrix
    {
        ...
        MATRIX_TEST_CALL(jmtxf_matrix_crs_build_row(system_matrix, 0, ROW_SIZE, idx, val));
    }
    {
        //  Interior rows
        enum
        ...
        for (uint32_t row = 1; row < n - 1; ++row)
        {
            ...
            idx[1] = row;
            idx[2] = row + 1;
            MATRIX_TEST_CALL(jmtxf_matrix_crs_build_row(system_matrix, row, ROW_SIZE, idx, val));
        }
    }
    {
        //  Last row
        ...
        MATRIX_TEST_CALL(jmtxf_matrix_crs_build_row(system_matrix, n - 1, ROW_SIZE, idx, val));
    }
    ...
    jmtxf_matrix_crs *l, *u;
    jmtxf_matrix_ccs *cu;

    MATRIX_TEST_CALL(jmtxf_decompose_ilu_crs(system_matrix, &l, &cu, NULL));

    MATRIX_TEST_CALL(jmtxf_convert_ccs_to_crs(cu, &u, NULL));
...
static float get_rms_error_for_element_count_brm(const unsigned n)
{
    ...
    jmtxf_matrix_brm *system_matrix;
    jmtx_result mtx_res;
    ...
    //  Build the matrix
    {
        ...
        jmtxf_matrix_brm_set_row(system_matrix, 0, val);
    }
    {
        //  Interior rows
        enum
        ...
        for (uint32_t row = 1; row < n - 1; ++row)
        {
            jmtxf_matrix_brm_set_row(system_matrix, row, val);
        }
    }
    {
        //  Last row
        ...
        jmtxf_matrix_brm_set_row(system_matrix, n - 1, val);
    }
    ...
    jmtxf_matrix_brm *l, *u;

    MATRIX_TEST_CALL(jmtxf_decompose_lu_brm(system_matrix, &l, &u, NULL));
    //    print_brm_matrix(l);
    //    print_brm_matrix(u);
