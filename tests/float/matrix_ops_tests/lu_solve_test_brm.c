...
int main()
{
    ...
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    MATRIX_TEST_CALL(jmtx_multiply_matrix_crs(lower, cu, &multiplied, NULL));
    ASSERT(mtx_res == JMTX_RESULT_SUCCESS);
    ...
    jmtx_solve_direct_lu_crs(lower, upper, y, x);
    jmtx_solve_direct_lu_brm(lower_brm, upper_brm, yb, xb);
