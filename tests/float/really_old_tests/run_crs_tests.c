...
int main()
{
    ...
    uint32_t funny = 0;
    jmtx_matrix_crs_set_entry(matrix, 1, 9, -100);
    jmtx_matrix_crs_set_entry(matrix, 6, 9, -200);
    jmtx_matrix_crs_set_entry(matrix, 2, 24, -300);
    jmtx_matrix_crs_apply_unary_fn(matrix, make_the_values_funnier, &funny);
    jmtx_matrix_crs_set_entry(matrix, 2, 9, -400);
    jmtx_matrix_crs_shrink(matrix);
    jmtx_matrix_crs_set_entry(matrix, 2, 1, -400);
    jmtx_matrix_crs_set_entry(matrix, 20, 1, -400);
    printf("\n\nBREAK 1\n");
    print_crs_matrix(matrix);
    printf("Row 2 is:\n");
    {
    ...
    jmtx_matrix_crs_copy(matrix, &tmp, NULL);
    err_count = 0;
    for (uint32_t i = 0; i < 25; ++i)
    {
        for (uint32_t j = 0; j < 25; ++j)
