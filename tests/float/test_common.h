...
struct jmtxf_matrix_brm_struct;
struct jmtx_matrix_cds_struct;
struct jmtxf_matrix_drm_struct;
void print_crs_matrix(const struct jmtxf_matrix_crs_struct *mtx);
void print_ccs_matrix(const struct jmtxf_matrix_ccs_struct *mtx);
void print_brm_matrix(const struct jmtxf_matrix_brm_struct *mtx);
...
#    define ASSERT(x)                                                                                                  \
        if (!(x))                                                                                                      \
        {                                                                                                              \
            fprintf(stderr, "Failed assertion \"" #x "\" on line %u, file %s\n", __LINE__, __FILE__);                  \
            DBG_BREAK;                                                                                                 \
            exit(EXIT_FAILURE);                                                                                        \
        }                                                                                                              \
        (void)0
