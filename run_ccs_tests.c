...
int main()
{
    ...
    jmtx_matrix_ccs_new(&matrix, 25, 25, 2, NULL);
    BEEF_CHECK(matrix);
    ...
    jmtx_matrix_ccs_remove_zeros(matrix);
    BEEF_CHECK(matrix);
    print_ccs_matrix(matrix);
    jmtx_matrix_ccs* matrix_1;
    jmtx_matrix_ccs* matrix_2;

    jmtx_matrix_ccs_new(&matrix_1, 5, 5, 0, NULL);
    ...
    jmtx_matrix_ccs_transpose(matrix_1, &matrix_2, NULL);
    ...
    jmtx_matrix_ccs* tmp;
    jmtx_matrix_ccs_transpose(matrix, &tmp, NULL);
    print_ccs_matrix(matrix);
    ...
    jmtx_matrix_ccs_destroy(tmp);

    jmtx_matrix_ccs_copy(matrix, &tmp, NULL);
    err_count = 0;
    for (uint32_t i = 0; i < 25; ++i)
    {
        for (uint32_t j = 0; j < 25; ++j)
    ...
    jmtx_matrix_ccs_destroy(matrix_1);
    jmtx_matrix_ccs_destroy(matrix_2);
...
//    printf("Matrix is currently using %zu bytes of memory\n", matrix_ccs_memory_usage(matrix));
    jmtx_matrix_ccs_destroy(matrix);
